...
// Live 2026-09-11 (req_011CewR5PEELa52aty766JLb): a paused web-research turn
// resumed without its container 400s "container_id is required when there are
// pending tool uses generated by code execution with tools".
func TestLLM_PauseTurnCarriesContainer(t *testing.T) {
	var n int32
	l, calls, bodies := newTestLLM(t, &memBudget{}, func(w http.ResponseWriter, r *http.Request) {
		if atomic.AddInt32(&n, 1) == 1 {
			io.WriteString(w, strings.Replace(strings.Replace(msgText, `"end_turn"`, `"pause_turn"`, 1),
				`"stop_sequence":null`, `"stop_sequence":null,"container":{"id":"cntr_abc","expires_at":"2026-09-11T12:00:00Z"}`, 1))
			return
		}
		io.WriteString(w, msgText)
	})
	...
	if strings.Contains((*bodies)[0], `"container"`) {
		t.Fatal("the first request has no container to name")
	}
	if !strings.Contains((*bodies)[1], `"container":"cntr_abc"`) {
		t.Fatalf("the resumed request must name the paused turn's container: %s", (*bodies)[1])
	}
}
