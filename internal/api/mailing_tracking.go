...
func (svc *MailingService) HandleTrackUnsubscribe(w http.ResponseWriter, r *http.Request) {
	...
	// Legacy 3-part token (org|campaign|subscriber): scope it to the
	// campaign's brand — exactly what the LIVE path does. The ALB forwards
	// every /track/* request, on every host, to the tracking service, whose
	// SQS consumer suppresses the originating brand only (operator doctrine
	// 2026-07-13: brands are separate senders). This handler used to widen
	// the same link to GLOBAL. When no brand resolves it stays global
	// (fail-safe: over-honour, never drop an opt-out).
	if len(parts) == 3 {
		var campFrom string
		svc.db.QueryRowContext(ctx, `SELECT COALESCE(from_email,'') FROM mailing_campaigns WHERE id = $1`, campaignID).Scan(&campFrom)
		brandRoot = brand.RootFromEmail(campFrom)
	}
...
func (svc *MailingService) HandleInboundMailtoUnsubscribe(w http.ResponseWriter, r *http.Request) {
	...
	// Signed mailto (2026-09-11): "<base64>.<sig>", sig = the same 16-hex
	// HMAC the /track/unsubscribe/{data}/{sig} links carry (worker
	// SignedMailtoToken). A present-but-wrong signature authorizes nothing.
	// LEGACY UNSIGNED tokens — every mailto header sent before this change —
	// are still honoured on purpose: this request is generated by the
	// recipient's own mailbox provider when the recipient asks to
	// unsubscribe, and refusing it would ignore a real unsubscribe. The
	// residual risk (a forger who knows org|campaign|subscriber UUIDs) can
	// only over-suppress, never drop an opt-out.
	if dot := strings.LastIndex(encoded, "."); dot > 0 {
		payload, sig := encoded[:dot], encoded[dot+1:]
		if !svc.verifySig(payload, sig) {
			log.Printf("INBOUND UNSUB: invalid mailto signature for recipient=%s — ignored", logger.RedactEmail(recipient))
			w.WriteHeader(http.StatusOK) // 200: SNS must not keep retrying a forgery
			return
		}
		encoded = payload
	}
	...
	// Global suppression THROUGH THE HUB (DB row + this task's in-memory set
	// in one call) so the send path enforces it without a restart. Until
	// 2026-09-11 this was a raw INSERT the hub only learned about at the next
	// boot. Scope stays GLOBAL — what this mailto leg has always done.
	emailLower := strings.ToLower(strings.TrimSpace(email))
	if svc.globalHub != nil {
		if _, err := svc.globalHub.Suppress(ctx, emailLower, "unsubscribe", "mailto_unsubscribe", isp, "", "", "", campaignID.String()); err != nil {
			log.Printf("ERROR: INBOUND MAILTO UNSUB hub Suppress failed for %s: %v", logger.RedactEmail(email), err)
		}
	} else {
		log.Printf("ERROR: INBOUND MAILTO UNSUB suppression hub not wired — DB-only write for %s, enforced after the next hub reload", logger.RedactEmail(email))
		md5Hash := fmt.Sprintf("%x", md5.Sum([]byte(emailLower)))
		svc.db.ExecContext(ctx, `
			INSERT INTO mailing_global_suppressions (id, organization_id, email, md5_hash, reason, source, isp, created_at)
