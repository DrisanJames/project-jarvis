...
// ---------------------------------------------------------------------------
// RewriteMoneyLinksToGateway
// ---------------------------------------------------------------------------

func TestRewriteMoneyLinksToGateway_SingleHref(t *testing.T) {
	in := `<a href="https://www.cratoolpro.com/ABC/DEF/?source_id=email&sub1=x">Go</a>`
	out, n := RewriteMoneyLinksToGateway(in, "discountblog.com", "auto-refi")
	if n != 1 {
		t.Fatalf("expected 1 rewrite, got %d", n)
	}
	if !strings.Contains(out, `href="https://discountblog.com/auto-refi"`) {
		t.Errorf("gateway href missing: %s", out)
	}
...
func TestRewriteMoneyLinksToGateway_MultipleHrefs(t *testing.T) {
	in := `<a href="http://cratoolpro.com/A/B/">one</a> mid ` +
		`<a href="https://www.cratoolpro.com/C/D/?q=1">two</a>`
	out, n := RewriteMoneyLinksToGateway(in, "quizfiesta.com", "capital-wallet")
	if n != 2 {
		t.Fatalf("expected 2 rewrites, got %d", n)
	}
	if strings.Count(out, `href="https://quizfiesta.com/capital-wallet"`) != 2 {
		t.Errorf("expected 2 gateway hrefs: %s", out)
	}
...
func TestRewriteMoneyLinksToGateway_NonCratoolproUntouched(t *testing.T) {
...
func TestRewriteMoneyLinksToGateway_IntegrationExcluded(t *testing.T) {
	// Mirrors the Python (?!integration/) negative lookahead.
	in := `<a href="https://www.cratoolpro.com/integration/postback?x=1">pixel</a>`
	out, n := RewriteMoneyLinksToGateway(in, "discountblog.com", "auto-refi")
	if n != 0 {
		t.Fatalf("integration path must NOT be rewritten, got n=%d", n)
	}
	if out != in {
		t.Errorf("integration href must be untouched:\n in=%s\nout=%s", in, out)
	}
	// Case-insensitive exclusion too.
	in2 := `<a href="https://cratoolpro.com/Integration/x">y</a>`
	out2, n2 := RewriteMoneyLinksToGateway(in2, "discountblog.com", "auto-refi")
	if n2 != 0 || out2 != in2 {
		t.Errorf("case-insensitive integration exclusion failed: n=%d out=%s", n2, out2)
	}
}

func TestRewriteMoneyLinksToGateway_MixedIntegrationAndOffer(t *testing.T) {
...
func TestRewriteMoneyLinksToGateway_Idempotent(t *testing.T) {
	in := gatewayCreativeHTML()
	once, n1 := RewriteMoneyLinksToGateway(in, "discountblog.com", "auto-refi")
	twice, n2 := RewriteMoneyLinksToGateway(once, "discountblog.com", "auto-refi")
	if n1 != 1 {
		t.Fatalf("first pass expected 1 rewrite, got %d", n1)
	}
	if n2 != 0 {
...
func TestRewriteMoneyLinksToGateway_EmptyInputsUnchanged(t *testing.T) {
	in := gatewayCreativeHTML()
	if out, n := RewriteMoneyLinksToGateway(in, "", "auto-refi"); n != 0 || out != in {
		t.Errorf("empty brandRoot must leave html unchanged (n=%d)", n)
	}
	if out, n := RewriteMoneyLinksToGateway(in, "discountblog.com", ""); n != 0 || out != in {
		t.Errorf("empty slug must leave html unchanged (n=%d)", n)
	}
}
