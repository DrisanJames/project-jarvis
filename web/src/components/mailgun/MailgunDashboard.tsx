...
import {
  ...
  faUserMinus, faPaperPlane, faArrowUp, faArrowDown, faTrophy, faChartBar,
  faShieldAlt
} from '@fortawesome/free-solid-svg-icons';
import { MetricCard, Card, CardHeader, CardBody, Loading, ErrorDisplay, StatusBadge, Table } from '../common';
import { useApi } from '../../hooks/useApi';
import { useDateFilter } from '../../context/DateFilterContext';
import type { MailgunDashboardData, MailgunISPMetrics, MailgunDomainMetrics } from '../../types';
...
export const MailgunDashboard: React.FC = () => {
  ...
  const apiUrl = `/api/mailgun/dashboard?start_date=${dateRange.startDate}&end_date=${dateRange.endDate}&range_type=${dateRange.type}`;

  const { data, loading, error, refetch } = useApi<MailgunDashboardData>(
    apiUrl,
  ...
  if (loading && !data) return <Loading message="Loading Mailgun dashboard..." />;
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <FontAwesomeIcon icon={faPaperPlane} style={{ color: 'var(--accent-orange)', fontSize: '24px' }} />
        <h2 style={{ margin: 0 }}>Mailgun Dashboard</h2>
        ...
        <span style={{ padding: '0.25rem 0.75rem', backgroundColor: 'rgba(251, 146, 60, 0.2)', color: 'var(--accent-orange)', borderRadius: '1rem', fontSize: '0.75rem', fontWeight: 500 }}>
          {data?.domain_metrics?.length ?? 0} domains
        </span>
      </div>
      ...
      {tickerItems.length > 0 && (
        <div style={{
          ...
          background: 'linear-gradient(90deg, rgba(251,146,60,0.05) 0%, rgba(251,146,60,0.02) 100%)',
          borderTop: '1px solid rgba(251,146,60,0.2)', borderBottom: '1px solid rgba(251,146,60,0.2)',
          borderRadius: '6px', display: 'flex', alignItems: 'center',
          scrollbarWidth: 'none',
        }}>
          <span style={{ padding: '0 0.75rem', fontSize: '0.65rem', fontWeight: 700, color: 'var(--accent-orange)', letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
            MAILGUN
          </span>
      ...
      <div className="grid grid-5 mb-6">
        ...
        <MetricCard label="Click Rate" value={summary?.click_rate ?? 0} format="percentage" />
        ...
        <Card className="mb-6">
          ...
          <CardBody>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
              {/* By Volume */}
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: 'var(--accent-orange)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {topPerformingISPs.byVolume.map((isp, idx) => (
                    <div key={isp.provider} style={{
                      ...
                      backgroundColor: idx === 0 ? 'rgba(251, 146, 60, 0.1)' : 'var(--bg-tertiary)',
                      borderRadius: '6px', borderLeft: idx === 0 ? '3px solid var(--accent-orange)' : 'none',
                    }}>
                      <span style={{ width: '20px', textAlign: 'center', fontWeight: 600, color: idx === 0 ? 'var(--accent-orange)' : 'var(--text-muted)', fontSize: '0.75rem' }}>
                        {idx === 0 ? <FontAwesomeIcon icon={faTrophy} /> : `#${idx + 1}`}
                      </span>
                      <span style={{ flex: 1, fontSize: '0.875rem', fontWeight: idx === 0 ? 600 : 400 }}>{isp.provider}</span>
                      <span style={{ fontFamily: 'monospace', fontSize: '0.8rem', fontWeight: 600, color: 'var(--accent-orange)' }}>{fmtNum(isp.volume)}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ── ISP Performance ── */}
      <Card className="mb-6">
        <CardHeader title="ISP Performance (Mailgun)" />
        <CardBody>
          <MailgunISPTable data={data?.isp_metrics ?? []} />
        </CardBody>
      </Card>
      ...
      <Card className="mb-6">
        ...
        <CardBody>
          <MailgunDomainTable data={data?.domain_metrics ?? []} />
        </CardBody>
      </Card>

      {/* ── Delivery Breakdown and Issues ── */}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {data.signals.top_issues.slice(0, 5).map((issue, idx) => (
                  <div key={idx} style={{ padding: '0.75rem', backgroundColor: 'var(--bg-tertiary)', borderRadius: '0.5rem' }}>
                    ...
                    <p style={{ fontSize: '0.813rem', color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {issue.description}
                    </p>
                  </div>
                ))}
...
const MetricRow: React.FC<MetricRowProps> = ({ icon, label, value, percentage, isNegative }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
    <div style={{ color: isNegative ? 'var(--accent-red)' : 'var(--accent-orange)', opacity: 0.8 }}>{icon}</div>
...
// ── ISP Table ──
const MailgunISPTable: React.FC<{ data: MailgunISPMetrics[] }> = ({ data }) => {
  if (data.length === 0) return <p style={{ color: 'var(--text-muted)', textAlign: 'center' }}>No ISP data available</p>;
  const columns = [
    { key: 'provider', header: 'Provider' },
    { key: 'status', header: 'Status', render: (item: MailgunISPMetrics) => <StatusBadge status={item.status} /> },
    { key: 'volume', header: 'Volume', render: (item: MailgunISPMetrics) => fmtNum(item.metrics.targeted) },
    { key: 'delivery', header: 'Delivery', render: (item: MailgunISPMetrics) => fmtPct(item.metrics.delivery_rate) },
    { key: 'opens', header: 'Opens', render: (item: MailgunISPMetrics) => fmtPct(item.metrics.open_rate) },
    { key: 'complaints', header: 'Complaints', render: (item: MailgunISPMetrics) => fmtPct(item.metrics.complaint_rate) },
    { key: 'bounces', header: 'Bounces', render: (item: MailgunISPMetrics) => fmtPct(item.metrics.bounce_rate) },
  ];
  return <Table columns={columns} data={data} keyExtractor={(item) => item.provider} />;
};

// ── Domain Table ──
const MailgunDomainTable: React.FC<{ data: MailgunDomainMetrics[] }> = ({ data }) => {
  if (data.length === 0) return <p style={{ color: 'var(--text-muted)', textAlign: 'center' }}>No domain data available</p>;
  const columns = [
    { key: 'domain', header: 'Domain' },
    { key: 'status', header: 'Status', render: (item: MailgunDomainMetrics) => <StatusBadge status={item.status} /> },
    { key: 'volume', header: 'Volume', render: (item: MailgunDomainMetrics) => fmtNum(item.metrics.targeted) },
    { key: 'delivery', header: 'Delivery', render: (item: MailgunDomainMetrics) => fmtPct(item.metrics.delivery_rate) },
    { key: 'complaints', header: 'Complaints', render: (item: MailgunDomainMetrics) => fmtPct(item.metrics.complaint_rate) },
    { key: 'bounces', header: 'Bounces', render: (item: MailgunDomainMetrics) => fmtPct(item.metrics.bounce_rate) },
  ];
  return <Table columns={columns} data={data} keyExtractor={(item) => item.domain} />;
};

export default MailgunDashboard;
