...
const REASON_LABELS: Record<string, string> = {
  ...
  subscriber_missing: 'Subscriber row no longer exists',
...
const REASON_HINTS: Record<string, string> = {
  ...
  no_event_in_window:
    'No mailing_tracking_events row matched this IP/time on a TruGreen link. Most often means the click came from a session not driven by our email (organic, retargeting, etc.).',
  invalid_ip: 'IP could not be parsed.',
  subscriber_missing: 'Tracking event existed but the subscriber row was hard-deleted.',
  db_error: 'Database error during lookup. Check server logs.',
};
    ...
    <div style={pageStyle}>
      <header style={headerStyle}>
        <div>
          ...
          <div style={{ color: 'rgba(180,210,240,0.65)', fontSize: 12, marginTop: 4 }}>
            Resolve Everflow click and conversion CSVs back to subscriber profiles
          </div>
        </div>
      ...
      <section style={cardStyle}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <FileSlot
            label="Everflow Clicks CSV"
            placeholder="trugreen-clicks.csv"
            file={clicksFile}
            onChange={setClicksFile}
          />
          <FileSlot
            label="Everflow Conversions CSV"
      ...
      {!result && !loading && (
        <section style={{ ...cardStyle, marginTop: 16, color: 'rgba(180,210,240,0.65)', fontSize: 13 }}>
          ...
          Drop the Everflow click and conversion exports above. We correlate each row's IP and
          timestamp against <code style={codeStyle}>mailing_tracking_events</code>, look for a
          matching click on a link that ILIKEs the offer pattern, then pull the subscriber
          profile from <code style={codeStyle}>mailing_subscribers</code>. Rows that don't
          match (Outlook scanners, missing tracking events, expired retention) are listed with
