...
var concurrentIndexSpecs = []struct {
	...
	{"idx_pcq_mailed_at", `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pcq_mailed_at ON partner_clean_queue (mailed_at) WHERE mailed_at IS NOT NULL`},
	// Per-lane dispatch/hygiene scans (§6.7: dataset_id = $1 AND mailed_at
	// range) and the §9.2 inactivity watch (bounded to trailing 48h).
	{"idx_pcq_dataset_mailed_at", `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pcq_dataset_mailed_at ON partner_clean_queue (dataset_id, mailed_at) WHERE mailed_at IS NOT NULL`},
	...
	{"idx_pcq_validated_at", `CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pcq_validated_at ON partner_clean_queue (validated_at) WHERE validated_at IS NOT NULL`},
...
// vocabulary CHECK — one source of truth, same pattern as
// ownedDomainsSeedSQL: the DDL can never drift from the platform ISP map
// because it IS the platform ISP map. Vocabulary (rev-4.1 §5.0 ruling,
// STOP-2 disposition): isp.AllGroups() UNION {isp.Other} on lane_isp rows,
// the empty-string sentinel on lane rows — GroupFromDomain returns "other"
// for the long tail while AllGroups() excludes it, so without 'other' the
// first long-tail row at P2 would violate the constraint. Shape matches the
// §5.0b widening entry exactly, so a fresh DB (this generator) and an
// already-shipped DB (dob_widen_isp_vocab_other) converge on the identical
// constraint. Parity pinned by TestObservatoryISPCheckMatchesAllGroups and
// the integration suite's pg_get_constraintdef comparison. Idempotent via
// the pg_constraint probe (the pdbb_* named-CHECK idiom above).
func observatoryISPVocabSQL(table, constraint string) string {
	groups := append(isp.AllGroups(), isp.Other)
	...
	return `DO $$ BEGIN
		IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='` + constraint + `') THEN
			ALTER TABLE ` + table + ` ADD CONSTRAINT ` + constraint + ` CHECK (
				(dimension_scope = 'lane' AND isp = '') OR
				(dimension_scope = 'lane_isp' AND isp IN (` + strings.Join(quoted, ", ") + `)));
		END IF;
	END $$`
}

// dobWidenISPVocabOtherSQL is the §5.0b widening migration, pinned verbatim
// from plan rev 4.1: commit 580f313 shipped dob_cohort_isp_vocab /
// dob_event_isp_vocab at the narrow (no-'other') vocabulary; this entry is
// pg_constraint-guarded drop-if-narrow / add-if-missing (idempotent; the
// fact tables are near-empty so validation is instant). A package-level
// constant so the §5.0 parity test can parse the vocabulary from the
// migration constant itself.
const dobWidenISPVocabOtherSQL = `DO $mig$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'dob_cohort_isp_vocab'
                 AND pg_get_constraintdef(oid) NOT LIKE '%''other''%') THEN
        ALTER TABLE partner_drip_send_cohort_daily DROP CONSTRAINT dob_cohort_isp_vocab;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'dob_cohort_isp_vocab') THEN
        ALTER TABLE partner_drip_send_cohort_daily ADD CONSTRAINT dob_cohort_isp_vocab CHECK (
            (dimension_scope = 'lane' AND isp = '') OR
            (dimension_scope = 'lane_isp' AND isp IN
             ('gmail','yahoo','aol','microsoft','apple','comcast','charter','att',
              'sbcglobal','cox','verizon','protonmail','zoho','other')));
    END IF;
    IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'dob_event_isp_vocab'
                 AND pg_get_constraintdef(oid) NOT LIKE '%''other''%') THEN
        ALTER TABLE partner_drip_event_daily DROP CONSTRAINT dob_event_isp_vocab;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'dob_event_isp_vocab') THEN
        ALTER TABLE partner_drip_event_daily ADD CONSTRAINT dob_event_isp_vocab CHECK (
            (dimension_scope = 'lane' AND isp = '') OR
            (dimension_scope = 'lane_isp' AND isp IN
             ('gmail','yahoo','aol','microsoft','apple','comcast','charter','att',
              'sbcglobal','cox','verizon','protonmail','zoho','other')));
...
// nxWaveIPRepointSQL points the nx wave-domain addresses at their
// '<brand><tok>-general-pool'. It runs TWICE per boot on purpose: once in
// place (nx_wave_ip_repoint_20260918, after the pools are created) and again
// as the LAST startup migration (nx_wave_ip_repoint_20260918_final_pass),
// because the legacy DO-blocks further down assign explicit 144.225.178.x
// addresses to '<brand>-{isp,general}-pool' by hard-coded lists on every boot
// and were undoing this repoint: six deploys on 2026-09-20/21 left 199 of 425
// nx addresses in an nx pool and 34,344 NX cold sends failed with "no sending
// IPs configured for profile" (brain #3702). Idempotent; the probe cannot
// classify it, so it costs one UPDATE per boot (rows already in place = 0).
const nxWaveIPRepointSQL = `
UPDATE mailing_ip_addresses a
SET pool_id = p.id, updated_at = NOW()
FROM (VALUES

    ('16.217.96.209', 'aadpg-general-pool'),
    ('16.217.96.216', 'aadpg-general-pool'),
    ('16.217.96.217', 'aadls-general-pool'),
    ('16.217.96.76', 'bccrp-general-pool'),
    ('16.217.96.77', 'bccrp-general-pool'),
    ('16.217.96.78', 'bccgk-general-pool'),
    ('16.217.96.79', 'bccgk-general-pool'),
    ('16.217.96.80', 'bccru-general-pool'),
    ('16.217.96.81', 'bccru-general-pool'),
    ('16.217.96.82', 'bccpz-general-pool'),
    ('16.217.96.83', 'bccpz-general-pool'),
    ('16.217.96.84', 'bccrk-general-pool'),
    ('16.217.96.85', 'bccrk-general-pool'),
    ('16.217.96.86', 'bccct-general-pool'),
    ('16.217.96.87', 'bccct-general-pool'),
    ('16.217.96.88', 'bccxa-general-pool'),
    ('16.217.96.89', 'bccxa-general-pool'),
    ('16.217.96.90', 'bccax-general-pool'),
    ('16.217.96.91', 'bccax-general-pool'),
    ('16.217.96.92', 'bccgp-general-pool'),
    ('16.217.96.93', 'bccgp-general-pool'),
    ('16.217.96.94', 'bccrs-general-pool'),
    ('16.217.96.95', 'bccrs-general-pool'),
    ('16.217.96.97', 'bccon-general-pool'),
    ('144.225.178.51', 'bwus-general-pool'),
    ('144.225.178.52', 'bwus-general-pool'),
    ('144.225.178.53', 'bwyr-general-pool'),
    ('144.225.178.90', 'cioj-general-pool'),
    ('144.225.178.96', 'cioj-general-pool'),
    ('144.225.178.114', 'cigu-general-pool'),
    ('144.225.178.54', 'cpur-general-pool'),
    ('144.225.178.118', 'cpur-general-pool'),
    ('144.225.178.119', 'cppp-general-pool'),
    ('144.225.178.0', 'dbpf-general-pool'),
    ('144.225.178.1', 'dbpf-general-pool'),
    ('144.225.178.2', 'dbgr-general-pool'),
    ('144.225.178.3', 'dbgr-general-pool'),
    ('144.225.178.4', 'dbil-general-pool'),
    ('144.225.178.5', 'dbil-general-pool'),
    ('144.225.178.6', 'dbal-general-pool'),
    ('144.225.178.7', 'dbal-general-pool'),
    ('144.225.178.8', 'dbae-general-pool'),
    ('144.225.178.9', 'dbae-general-pool'),
    ('144.225.178.10', 'dbam-general-pool'),
    ('144.225.178.11', 'dbam-general-pool'),
    ('144.225.178.12', 'dbqi-general-pool'),
    ('144.225.178.13', 'dbqi-general-pool'),
    ('144.225.178.14', 'dbiy-general-pool'),
    ('144.225.178.15', 'dbiy-general-pool'),
    ('144.225.178.16', 'dbaa-general-pool'),
    ('144.225.178.17', 'dbaa-general-pool'),
    ('144.225.178.18', 'dbjt-general-pool'),
    ('144.225.178.19', 'dbjt-general-pool'),
    ('144.225.178.20', 'dbdc-general-pool'),
    ('144.225.178.21', 'dbdc-general-pool'),
    ('144.225.178.22', 'dbwn-general-pool'),
    ('144.225.178.23', 'dbwn-general-pool'),
    ('144.225.178.24', 'dbnn-general-pool'),
    ('144.225.178.25', 'dbnn-general-pool'),
    ('144.225.178.27', 'dbdx-general-pool'),
    ('144.225.178.28', 'dbdx-general-pool'),
    ('144.225.178.29', 'dbmh-general-pool'),
    ('144.225.178.30', 'dbmh-general-pool'),
    ('144.225.178.31', 'dbkm-general-pool'),
    ('144.225.178.33', 'dbkm-general-pool'),
    ('144.225.178.34', 'dbhk-general-pool'),
    ('144.225.178.35', 'dbhk-general-pool'),
    ('144.225.178.36', 'dbez-general-pool'),
    ('144.225.178.37', 'dbez-general-pool'),
    ('144.225.178.38', 'dbjy-general-pool'),
    ('144.225.178.39', 'dbjy-general-pool'),
    ('144.225.178.40', 'dbsx-general-pool'),
    ('144.225.178.41', 'dbsx-general-pool'),
    ('144.225.178.42', 'dbcm-general-pool'),
    ('144.225.178.43', 'dbcm-general-pool'),
    ('144.225.178.44', 'dbsq-general-pool'),
    ('144.225.178.45', 'dbsq-general-pool'),
    ('144.225.178.46', 'dbfp-general-pool'),
    ('144.225.178.47', 'dbfp-general-pool'),
    ('144.225.178.48', 'dbwm-general-pool'),
    ('144.225.178.49', 'dbwm-general-pool'),
    ('144.225.178.55', 'dbgs-general-pool'),
    ('144.225.178.115', 'fcot-general-pool'),
    ('144.225.178.116', 'fcot-general-pool'),
    ('144.225.178.117', 'fcrc-general-pool'),
    ('16.217.96.164', 'fthuz-general-pool'),
    ('16.217.96.165', 'fthuz-general-pool'),
    ('16.217.96.166', 'fthho-general-pool'),
    ('16.217.96.167', 'fthho-general-pool'),
    ('16.217.96.168', 'fthag-general-pool'),
    ('16.217.96.169', 'fthag-general-pool'),
    ('16.217.96.170', 'fthnw-general-pool'),
    ('16.217.96.171', 'fthnw-general-pool'),
    ('16.217.96.172', 'fthnd-general-pool'),
    ('16.217.96.173', 'fthnd-general-pool'),
    ('16.217.96.174', 'fthto-general-pool'),
    ('16.217.96.175', 'fthto-general-pool'),
    ('16.217.96.176', 'fthpx-general-pool'),
    ('16.217.96.177', 'fthpx-general-pool'),
    ('16.217.96.178', 'fthmz-general-pool'),
    ('16.217.96.179', 'fthmz-general-pool'),
    ('16.217.96.180', 'fthgm-general-pool'),
    ('16.217.96.181', 'fthgm-general-pool'),
    ('16.217.96.182', 'fthma-general-pool'),
    ('16.217.96.183', 'fthma-general-pool'),
    ('16.217.96.185', 'fthvp-general-pool'),
    ('16.217.96.231', 'hfcbl-general-pool'),
    ('16.217.96.238', 'hfcbl-general-pool'),
    ('16.217.96.239', 'hfcox-general-pool'),
    ('16.217.96.142', 'hljzk-general-pool'),
    ('16.217.96.143', 'hljzk-general-pool'),
    ('16.217.96.144', 'hljfr-general-pool'),
    ('16.217.96.145', 'hljfr-general-pool'),
    ('16.217.96.146', 'hljyh-general-pool'),
    ('16.217.96.147', 'hljyh-general-pool'),
    ('16.217.96.148', 'hljhi-general-pool'),
    ('16.217.96.149', 'hljhi-general-pool'),
    ('16.217.96.150', 'hljww-general-pool'),
    ('16.217.96.151', 'hljww-general-pool'),
    ('16.217.96.152', 'hljdk-general-pool'),
    ('16.217.96.153', 'hljdk-general-pool'),
    ('16.217.96.154', 'hljfz-general-pool'),
    ('16.217.96.155', 'hljfz-general-pool'),
    ('16.217.96.156', 'hljvv-general-pool'),
    ('16.217.96.157', 'hljvv-general-pool'),
    ('16.217.96.158', 'hljzz-general-pool'),
    ('16.217.96.159', 'hljzz-general-pool'),
    ('16.217.96.160', 'hljfk-general-pool'),
    ('16.217.96.161', 'hljfk-general-pool'),
    ('16.217.96.162', 'hljqk-general-pool'),
    ('16.217.96.163', 'hljqk-general-pool'),
    ('144.225.178.128', 'htjk-general-pool'),
    ('144.225.178.129', 'htjk-general-pool'),
    ('144.225.178.130', 'htps-general-pool'),
    ('144.225.178.131', 'htps-general-pool'),
    ('144.225.178.132', 'htux-general-pool'),
    ('144.225.178.133', 'htux-general-pool'),
    ('144.225.178.134', 'htck-general-pool'),
    ('144.225.178.135', 'htck-general-pool'),
    ('144.225.178.136', 'htqn-general-pool'),
    ('144.225.178.137', 'htqn-general-pool'),
    ('144.225.178.138', 'htfi-general-pool'),
    ('144.225.178.139', 'htfi-general-pool'),
    ('144.225.178.140', 'htbb-general-pool'),
    ('144.225.178.142', 'htbb-general-pool'),
    ('144.225.178.143', 'htat-general-pool'),
    ('144.225.178.144', 'htat-general-pool'),
    ('144.225.178.145', 'htna-general-pool'),
    ('144.225.178.146', 'htna-general-pool'),
    ('144.225.178.147', 'htwe-general-pool'),
    ('144.225.178.148', 'htwe-general-pool'),
    ('144.225.178.149', 'hthw-general-pool'),
    ('144.225.178.150', 'hthw-general-pool'),
    ('144.225.178.151', 'htub-general-pool'),
    ('144.225.178.152', 'htub-general-pool'),
    ('144.225.178.153', 'htye-general-pool'),
    ('144.225.178.154', 'htye-general-pool'),
    ('144.225.178.155', 'htmu-general-pool'),
    ('144.225.178.156', 'htmu-general-pool'),
    ('144.225.178.157', 'htij-general-pool'),
    ('144.225.178.166', 'htij-general-pool'),
    ('144.225.178.167', 'htib-general-pool'),
    ('144.225.178.168', 'htib-general-pool'),
    ('144.225.178.169', 'htyu-general-pool'),
    ('144.225.178.170', 'htyu-general-pool'),
    ('144.225.178.171', 'htwc-general-pool'),
    ('144.225.178.172', 'htwc-general-pool'),
    ('144.225.178.173', 'htjj-general-pool'),
    ('144.225.178.174', 'htjj-general-pool'),
    ('144.225.178.175', 'htly-general-pool'),
    ('144.225.178.176', 'htly-general-pool'),
    ('144.225.178.177', 'htvr-general-pool'),
    ('144.225.178.178', 'htvr-general-pool'),
    ('144.225.178.179', 'htph-general-pool'),
    ('16.217.96.186', 'htmnz-general-pool'),
    ('16.217.96.187', 'htmnz-general-pool'),
    ('16.217.96.188', 'htmwb-general-pool'),
    ('16.217.96.189', 'htmwb-general-pool'),
    ('16.217.96.190', 'htmhj-general-pool'),
    ('16.217.96.191', 'htmhj-general-pool'),
    ('16.217.96.192', 'htmhh-general-pool'),
    ('16.217.96.193', 'htmhh-general-pool'),
    ('16.217.96.194', 'htmju-general-pool'),
    ('16.217.96.195', 'htmju-general-pool'),
    ('16.217.96.196', 'htmdv-general-pool'),
    ('16.217.96.197', 'htmdv-general-pool'),
    ('16.217.96.198', 'htmpq-general-pool'),
    ('16.217.96.199', 'htmpq-general-pool'),
    ('16.217.96.200', 'htmnq-general-pool'),
    ('16.217.96.201', 'htmnq-general-pool'),
    ('16.217.96.202', 'htmld-general-pool'),
    ('16.217.96.203', 'htmld-general-pool'),
    ('16.217.96.204', 'htmbw-general-pool'),
    ('16.217.96.205', 'htmbw-general-pool'),
    ('16.217.96.207', 'htmik-general-pool'),
    ('144.225.178.188', 'hwed-general-pool'),
    ('144.225.178.158', 'lpif-general-pool'),
    ('144.225.178.165', 'lpif-general-pool'),
    ('144.225.178.185', 'lpug-general-pool'),
    ('15.204.38.175', 'mhtu-general-pool'),
    ('144.225.178.192', 'mhtu-general-pool'),
    ('144.225.178.193', 'mhaw-general-pool'),
    ('144.225.178.194', 'mhaw-general-pool'),
    ('144.225.178.195', 'mhuv-general-pool'),
    ('144.225.178.196', 'mhuv-general-pool'),
    ('144.225.178.197', 'mhnl-general-pool'),
    ('144.225.178.198', 'mhnl-general-pool'),
    ('144.225.178.199', 'mhkx-general-pool'),
    ('144.225.178.200', 'mhkx-general-pool'),
    ('144.225.178.201', 'mhlr-general-pool'),
    ('144.225.178.204', 'mhlr-general-pool'),
    ('144.225.178.206', 'mhnk-general-pool'),
    ('144.225.178.207', 'mhnk-general-pool'),
    ('144.225.178.208', 'mhfe-general-pool'),
    ('144.225.178.209', 'mhfe-general-pool'),
    ('144.225.178.210', 'mhro-general-pool'),
    ('144.225.178.211', 'mhro-general-pool'),
    ('144.225.178.212', 'mhtc-general-pool'),
    ('144.225.178.213', 'mhtc-general-pool'),
    ('144.225.178.214', 'mhba-general-pool'),
    ('144.225.178.215', 'mhba-general-pool'),
    ('144.225.178.216', 'mhbr-general-pool'),
    ('144.225.178.217', 'mhbr-general-pool'),
    ('144.225.178.218', 'mhpd-general-pool'),
    ('144.225.178.219', 'mhpd-general-pool'),
    ('144.225.178.220', 'mhex-general-pool'),
    ('144.225.178.221', 'mhex-general-pool'),
    ('144.225.178.223', 'mhol-general-pool'),
    ('144.225.178.227', 'mhol-general-pool'),
    ('144.225.178.228', 'mhko-general-pool'),
    ('144.225.178.230', 'mhko-general-pool'),
    ('144.225.178.231', 'mhpc-general-pool'),
    ('144.225.178.232', 'mhpc-general-pool'),
    ('144.225.178.233', 'mhzu-general-pool'),
    ('144.225.178.234', 'mhzu-general-pool'),
    ('144.225.178.235', 'mhau-general-pool'),
    ('144.225.178.236', 'mhau-general-pool'),
    ('144.225.178.237', 'mhqt-general-pool'),
    ('144.225.178.238', 'mhqt-general-pool'),
    ('144.225.178.239', 'mhwg-general-pool'),
    ('144.225.178.240', 'mhwg-general-pool'),
    ('144.225.178.241', 'mhtr-general-pool'),
    ('144.225.178.242', 'mhtr-general-pool'),
    ('144.225.178.243', 'mhfw-general-pool'),
    ('144.225.178.244', 'mhfw-general-pool'),
    ('144.225.178.245', 'mhix-general-pool'),
    ('144.225.178.246', 'mhix-general-pool'),
    ('144.225.178.247', 'mhlg-general-pool'),
    ('144.225.178.248', 'mhlg-general-pool'),
    ('16.217.96.10', 'mpfwj-general-pool'),
    ('16.217.96.11', 'mpfwj-general-pool'),
    ('16.217.96.12', 'mpfjl-general-pool'),
    ('16.217.96.13', 'mpfjl-general-pool'),
    ('16.217.96.14', 'mpfee-general-pool'),
    ('16.217.96.15', 'mpfee-general-pool'),
    ('16.217.96.16', 'mpfws-general-pool'),
    ('16.217.96.17', 'mpfws-general-pool'),
    ('16.217.96.18', 'mpfkq-general-pool'),
    ('16.217.96.19', 'mpfkq-general-pool'),
    ('16.217.96.20', 'mpfxr-general-pool'),
    ('16.217.96.21', 'mpfxr-general-pool'),
    ('16.217.96.22', 'mpfwo-general-pool'),
    ('16.217.96.23', 'mpfwo-general-pool'),
    ('16.217.96.24', 'mpfep-general-pool'),
    ('16.217.96.25', 'mpfep-general-pool'),
    ('16.217.96.26', 'mpfou-general-pool'),
    ('16.217.96.27', 'mpfou-general-pool'),
    ('16.217.96.28', 'mpfyn-general-pool'),
    ('16.217.96.29', 'mpfyn-general-pool'),
    ('16.217.96.31', 'mpfec-general-pool'),
    ('144.225.178.26', 'mrvi-general-pool'),
    ('144.225.178.32', 'mrvi-general-pool'),
    ('144.225.178.50', 'mrdg-general-pool'),
    ('16.217.96.32', 'pdpy-general-pool'),
    ('16.217.96.33', 'pdpy-general-pool'),
    ('16.217.96.34', 'pdzv-general-pool'),
    ('16.217.96.35', 'pdzv-general-pool'),
    ('16.217.96.36', 'pdlh-general-pool'),
    ('16.217.96.37', 'pdlh-general-pool'),
    ('16.217.96.38', 'pdoh-general-pool'),
    ('16.217.96.39', 'pdoh-general-pool'),
    ('16.217.96.40', 'pdhm-general-pool'),
    ('16.217.96.41', 'pdhm-general-pool'),
    ('16.217.96.42', 'pdvj-general-pool'),
    ('16.217.96.43', 'pdvj-general-pool'),
    ('16.217.96.44', 'pdab-general-pool'),
    ('16.217.96.45', 'pdab-general-pool'),
    ('16.217.96.46', 'pdxn-general-pool'),
    ('16.217.96.47', 'pdxn-general-pool'),
    ('16.217.96.48', 'pdsh-general-pool'),
    ('16.217.96.49', 'pdsh-general-pool'),
    ('16.217.96.50', 'pdof-general-pool'),
    ('16.217.96.51', 'pdof-general-pool'),
    ('16.217.96.52', 'pdcv-general-pool'),
    ('16.217.96.53', 'pdcv-general-pool'),
    ('15.204.22.191', 'qfbe-general-pool'),
    ('144.225.178.64', 'qfbe-general-pool'),
    ('144.225.178.65', 'qfcs-general-pool'),
    ('144.225.178.66', 'qfcs-general-pool'),
    ('144.225.178.67', 'qfaf-general-pool'),
    ('144.225.178.68', 'qfaf-general-pool'),
    ('144.225.178.69', 'qfua-general-pool'),
    ('144.225.178.70', 'qfua-general-pool'),
    ('144.225.178.71', 'qfta-general-pool'),
    ('144.225.178.72', 'qfta-general-pool'),
    ('144.225.178.73', 'qfqj-general-pool'),
    ('144.225.178.74', 'qfqj-general-pool'),
    ('144.225.178.75', 'qfbs-general-pool'),
    ('144.225.178.76', 'qfbs-general-pool'),
    ('144.225.178.77', 'qfjw-general-pool'),
    ('144.225.178.78', 'qfjw-general-pool'),
    ('144.225.178.79', 'qfdl-general-pool'),
    ('144.225.178.80', 'qfdl-general-pool'),
    ('144.225.178.81', 'qfdm-general-pool'),
    ('144.225.178.82', 'qfdm-general-pool'),
    ('144.225.178.83', 'qfdt-general-pool'),
    ('144.225.178.84', 'qfdt-general-pool'),
    ('144.225.178.85', 'qfrv-general-pool'),
    ('144.225.178.86', 'qfrv-general-pool'),
    ('144.225.178.87', 'qfyp-general-pool'),
    ('144.225.178.88', 'qfyp-general-pool'),
    ('144.225.178.89', 'qfya-general-pool'),
    ('144.225.178.91', 'qfya-general-pool'),
    ('144.225.178.92', 'qfad-general-pool'),
    ('144.225.178.93', 'qfad-general-pool'),
    ('144.225.178.94', 'qfwt-general-pool'),
    ('144.225.178.95', 'qfwt-general-pool'),
    ('144.225.178.97', 'qfrm-general-pool'),
    ('144.225.178.98', 'qfrm-general-pool'),
    ('144.225.178.99', 'qfav-general-pool'),
    ('144.225.178.100', 'qfav-general-pool'),
    ('144.225.178.101', 'qfqg-general-pool'),
    ('144.225.178.102', 'qfqg-general-pool'),
    ('144.225.178.103', 'qflo-general-pool'),
    ('144.225.178.104', 'qflo-general-pool'),
    ('144.225.178.105', 'qfmb-general-pool'),
    ('144.225.178.106', 'qfmb-general-pool'),
    ('144.225.178.107', 'qfzt-general-pool'),
    ('144.225.178.108', 'qfzt-general-pool'),
    ('144.225.178.109', 'qfsi-general-pool'),
    ('144.225.178.110', 'qfsi-general-pool'),
    ('144.225.178.111', 'qfob-general-pool'),
    ('144.225.178.112', 'qfob-general-pool'),
    ('144.225.178.113', 'qfgd-general-pool'),
    ('144.225.178.120', 'qfgd-general-pool'),
    ('144.225.178.222', 'rbhp-general-pool'),
    ('144.225.178.229', 'rbhp-general-pool'),
    ('144.225.178.249', 'rbaz-general-pool'),
    ('144.225.178.189', 'rruc-general-pool'),
    ('144.225.178.190', 'rruc-general-pool'),
    ('144.225.178.191', 'rrgo-general-pool'),
    ('16.217.96.54', 'trgz-general-pool'),
    ('16.217.96.55', 'trgz-general-pool'),
    ('16.217.96.56', 'troi-general-pool'),
    ('16.217.96.57', 'troi-general-pool'),
    ('16.217.96.58', 'trvn-general-pool'),
    ('16.217.96.59', 'trvn-general-pool'),
    ('16.217.96.60', 'trks-general-pool'),
    ('16.217.96.61', 'trks-general-pool'),
    ('16.217.96.62', 'trkh-general-pool'),
    ('16.217.96.63', 'trkh-general-pool'),
    ('16.217.96.64', 'trlz-general-pool'),
    ('16.217.96.65', 'trlz-general-pool'),
    ('16.217.96.66', 'trhc-general-pool'),
    ('16.217.96.67', 'trhc-general-pool'),
    ('16.217.96.68', 'trow-general-pool'),
    ('16.217.96.69', 'trow-general-pool'),
    ('16.217.96.70', 'tryb-general-pool'),
    ('16.217.96.71', 'tryb-general-pool'),
    ('16.217.96.72', 'trdr-general-pool'),
    ('16.217.96.73', 'trdr-general-pool'),
    ('16.217.96.74', 'trdn-general-pool'),
    ('16.217.96.75', 'trdn-general-pool'),
    ('144.225.178.250', 'ttqb-general-pool'),
    ('144.225.178.251', 'ttqb-general-pool'),
    ('144.225.178.252', 'ttsr-general-pool'),
    ('16.217.96.98', 'usfxv-general-pool'),
    ('16.217.96.99', 'usfxv-general-pool'),
    ('16.217.96.100', 'usfge-general-pool'),
    ('16.217.96.101', 'usfge-general-pool'),
    ('16.217.96.102', 'usftx-general-pool'),
    ('16.217.96.103', 'usftx-general-pool'),
    ('16.217.96.104', 'usfzh-general-pool'),
    ('16.217.96.105', 'usfzh-general-pool'),
    ('16.217.96.106', 'usfis-general-pool'),
    ('16.217.96.107', 'usfis-general-pool'),
    ('16.217.96.108', 'usfen-general-pool'),
    ('16.217.96.109', 'usfen-general-pool'),
    ('16.217.96.110', 'usfvg-general-pool'),
    ('16.217.96.111', 'usfvg-general-pool'),
    ('16.217.96.112', 'usfrx-general-pool'),
    ('16.217.96.113', 'usfrx-general-pool'),
    ('16.217.96.114', 'usffq-general-pool'),
    ('16.217.96.115', 'usffq-general-pool'),
    ('16.217.96.116', 'usfcy-general-pool'),
    ('16.217.96.117', 'usfcy-general-pool'),
    ('16.217.96.118', 'usfhz-general-pool'),
    ('16.217.96.119', 'usfhz-general-pool'),
    ('144.225.178.202', 'wfay-general-pool'),
    ('144.225.178.203', 'wfay-general-pool'),
    ('144.225.178.205', 'wfef-general-pool'),
    ('16.217.96.120', 'yfbud-general-pool'),
    ('16.217.96.121', 'yfbud-general-pool'),
    ('16.217.96.122', 'yfbnm-general-pool'),
    ('16.217.96.123', 'yfbnm-general-pool'),
    ('16.217.96.124', 'yfbuw-general-pool'),
    ('16.217.96.125', 'yfbuw-general-pool'),
    ('16.217.96.126', 'yfbvs-general-pool'),
    ('16.217.96.127', 'yfbvs-general-pool'),
    ('16.217.96.128', 'yfbjn-general-pool'),
    ('16.217.96.129', 'yfbjn-general-pool'),
    ('16.217.96.130', 'yfbeb-general-pool'),
    ('16.217.96.131', 'yfbeb-general-pool'),
    ('16.217.96.132', 'yfbbx-general-pool'),
    ('16.217.96.133', 'yfbbx-general-pool'),
    ('16.217.96.134', 'yfbzw-general-pool'),
    ('16.217.96.135', 'yfbzw-general-pool'),
    ('16.217.96.136', 'yfbgb-general-pool'),
    ('16.217.96.137', 'yfbgb-general-pool'),
    ('16.217.96.138', 'yfbej-general-pool'),
    ('16.217.96.139', 'yfbej-general-pool'),
    ('16.217.96.141', 'yfbor-general-pool'),
    ('144.225.178.253', 'yipl-general-pool'),
    ('144.225.178.254', 'yipl-general-pool'),
    ('144.225.178.255', 'yiut-general-pool')
) AS v(ip, pool_name)
JOIN mailing_ip_pools p ON p.name = v.pool_name
                       AND p.organization_id = '00000000-0000-0000-0000-000000000001'::uuid
WHERE a.ip_address = v.ip::inet
  AND a.organization_id = '00000000-0000-0000-0000-000000000001'::uuid
  AND (a.pool_id IS DISTINCT FROM p.id);
`
...
func runStartupMigrations(db *sql.DB) {
	...
	migrations := []struct {
		...
		{"create_stream_broadcast_config", `CREATE TABLE IF NOT EXISTS mailing_stream_broadcast_config (
			...
			throttle_hours  INTEGER NOT NULL DEFAULT 12,
			label           TEXT NOT NULL DEFAULT '',
		...
		{"seed_stream_broadcast_config", `
			...
			CROSS JOIN (VALUES
				...
				('remodel', TRUE, 5000, '{"gmail":0}', '3-day-blinds', 12,
		...
		// === nx wave (2026-09-18): 223 additive sending domains, 2 idle IPs each ===
		// Generated by agents/jobs/nx_profile_seed.py. ONE idempotent 223-row INSERT:
		// 223 separate statements would each burn a slot in the 5s-per-statement
		// budget, where a timeout logs "skipped, will retry next boot" forever.
		// status='draft' deliberately — the planner filters status='active', so a
		// draft profile cannot be picked up for a live send until its pool, its DKIM
		// key and its proof send are real.
		{"nx_wave_sending_profiles_20260918", `
INSERT INTO mailing_sending_profiles (
    id, organization_id, name, vendor_type, from_name, from_email, reply_email,
    sending_domain, smtp_host, smtp_port, tracking_domain, hourly_limit,
    daily_limit, pool_prefix, routing_mode, via_ses, status, is_default,
    created_at, updated_at)
SELECT gen_random_uuid(), v.org::uuid, v.name, v.vendor, v.from_name, v.from_email,
       v.reply_email, v.sending_domain, v.smtp_host, v.smtp_port, v.tracking_domain,
       v.hourly_limit, v.daily_limit, v.pool_prefix, v.routing_mode, v.via_ses,
       v.status, v.is_default, NOW(), NOW()
FROM (VALUES
    ('00000000-0000-0000-0000-000000000001', 'NX AAD pg', 'pmta', 'Dana @ American Auto & Driver', 'hello@pg.aadwd.com', 'reply@pg.aadwd.com', 'pg.aadwd.com', '40.160.129.116', 587, 'trk.em.aadwd.com', 1500, 12000, 'aadpg', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX AAD ls', 'pmta', 'Dana @ American Auto & Driver', 'hello@ls.aadwd.com', 'reply@ls.aadwd.com', 'ls.aadwd.com', '40.160.129.116', 587, 'trk.em.aadwd.com', 1500, 12000, 'aadls', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC rp', 'pmta', 'Grace @ Best Credit Care', 'hello@rp.bestcreditcare.com', 'reply@rp.bestcreditcare.com', 'rp.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccrp', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC gk', 'pmta', 'Grace @ Best Credit Care', 'hello@gk.bestcreditcare.com', 'reply@gk.bestcreditcare.com', 'gk.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccgk', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC ru', 'pmta', 'Grace @ Best Credit Care', 'hello@ru.bestcreditcare.com', 'reply@ru.bestcreditcare.com', 'ru.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccru', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC pz', 'pmta', 'Grace @ Best Credit Care', 'hello@pz.bestcreditcare.com', 'reply@pz.bestcreditcare.com', 'pz.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccpz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC rk', 'pmta', 'Grace @ Best Credit Care', 'hello@rk.bestcreditcare.com', 'reply@rk.bestcreditcare.com', 'rk.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccrk', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC ct', 'pmta', 'Grace @ Best Credit Care', 'hello@ct.bestcreditcare.com', 'reply@ct.bestcreditcare.com', 'ct.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccct', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC xa', 'pmta', 'Grace @ Best Credit Care', 'hello@xa.bestcreditcare.com', 'reply@xa.bestcreditcare.com', 'xa.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccxa', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC ax', 'pmta', 'Grace @ Best Credit Care', 'hello@ax.bestcreditcare.com', 'reply@ax.bestcreditcare.com', 'ax.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccax', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC gp', 'pmta', 'Grace @ Best Credit Care', 'hello@gp.bestcreditcare.com', 'reply@gp.bestcreditcare.com', 'gp.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccgp', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC rs', 'pmta', 'Grace @ Best Credit Care', 'hello@rs.bestcreditcare.com', 'reply@rs.bestcreditcare.com', 'rs.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccrs', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BCC on', 'pmta', 'Grace @ Best Credit Care', 'hello@on.bestcreditcare.com', 'reply@on.bestcreditcare.com', 'on.bestcreditcare.com', '40.160.129.116', 587, 'trk.em.bestcreditcare.com', 1500, 12000, 'bccon', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BW us', 'pmta', 'Marcus @ Business Weekly Pro', 'hello@us.businessweeklypro.com', 'reply@us.businessweeklypro.com', 'us.businessweeklypro.com', '15.204.101.125', 587, 't.em.businessweeklypro.com', 1500, 12000, 'bwus', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX BW yr', 'pmta', 'Marcus @ Business Weekly Pro', 'hello@yr.businessweeklypro.com', 'reply@yr.businessweeklypro.com', 'yr.businessweeklypro.com', '15.204.101.125', 587, 't.em.businessweeklypro.com', 1500, 12000, 'bwyr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX CI oj', 'pmta', 'Maria @ Casa Insure', 'hello@oj.casainsure.com', 'reply@oj.casainsure.com', 'oj.casainsure.com', '15.204.101.125', 587, 't.em.casainsure.com', 1500, 12000, 'cioj', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX CI gu', 'pmta', 'Maria @ Casa Insure', 'hello@gu.casainsure.com', 'reply@gu.casainsure.com', 'gu.casainsure.com', '15.204.101.125', 587, 't.em.casainsure.com', 1500, 12000, 'cigu', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX CP ur', 'pmta', 'Diane @ Consumer Pro', 'hello@ur.consumerpro.net', 'reply@ur.consumerpro.net', 'ur.consumerpro.net', '15.204.101.125', 587, 't.em.consumerpro.net', 1500, 12000, 'cpur', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX CP pp', 'pmta', 'Diane @ Consumer Pro', 'hello@pp.consumerpro.net', 'reply@pp.consumerpro.net', 'pp.consumerpro.net', '15.204.101.125', 587, 't.em.consumerpro.net', 1500, 12000, 'cppp', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB pf', 'pmta', 'Jamie @ Discount Blog', 'hello@pf.discountblog.com', 'reply@pf.discountblog.com', 'pf.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbpf', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB gr', 'pmta', 'Jamie @ Discount Blog', 'hello@gr.discountblog.com', 'reply@gr.discountblog.com', 'gr.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbgr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB il', 'pmta', 'Jamie @ Discount Blog', 'hello@il.discountblog.com', 'reply@il.discountblog.com', 'il.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbil', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB al', 'pmta', 'Jamie @ Discount Blog', 'hello@al.discountblog.com', 'reply@al.discountblog.com', 'al.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbal', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB ae', 'pmta', 'Jamie @ Discount Blog', 'hello@ae.discountblog.com', 'reply@ae.discountblog.com', 'ae.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbae', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB am', 'pmta', 'Jamie @ Discount Blog', 'hello@am.discountblog.com', 'reply@am.discountblog.com', 'am.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbam', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB qi', 'pmta', 'Jamie @ Discount Blog', 'hello@qi.discountblog.com', 'reply@qi.discountblog.com', 'qi.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbqi', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB iy', 'pmta', 'Jamie @ Discount Blog', 'hello@iy.discountblog.com', 'reply@iy.discountblog.com', 'iy.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbiy', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB aa', 'pmta', 'Jamie @ Discount Blog', 'hello@aa.discountblog.com', 'reply@aa.discountblog.com', 'aa.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbaa', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB jt', 'pmta', 'Jamie @ Discount Blog', 'hello@jt.discountblog.com', 'reply@jt.discountblog.com', 'jt.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbjt', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB dc', 'pmta', 'Jamie @ Discount Blog', 'hello@dc.discountblog.com', 'reply@dc.discountblog.com', 'dc.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbdc', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB wn', 'pmta', 'Jamie @ Discount Blog', 'hello@wn.discountblog.com', 'reply@wn.discountblog.com', 'wn.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbwn', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB nn', 'pmta', 'Jamie @ Discount Blog', 'hello@nn.discountblog.com', 'reply@nn.discountblog.com', 'nn.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbnn', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB dx', 'pmta', 'Jamie @ Discount Blog', 'hello@dx.discountblog.com', 'reply@dx.discountblog.com', 'dx.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbdx', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB mh', 'pmta', 'Jamie @ Discount Blog', 'hello@mh.discountblog.com', 'reply@mh.discountblog.com', 'mh.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbmh', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB km', 'pmta', 'Jamie @ Discount Blog', 'hello@km.discountblog.com', 'reply@km.discountblog.com', 'km.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbkm', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB hk', 'pmta', 'Jamie @ Discount Blog', 'hello@hk.discountblog.com', 'reply@hk.discountblog.com', 'hk.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbhk', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB ez', 'pmta', 'Jamie @ Discount Blog', 'hello@ez.discountblog.com', 'reply@ez.discountblog.com', 'ez.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbez', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB jy', 'pmta', 'Jamie @ Discount Blog', 'hello@jy.discountblog.com', 'reply@jy.discountblog.com', 'jy.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbjy', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB sx', 'pmta', 'Jamie @ Discount Blog', 'hello@sx.discountblog.com', 'reply@sx.discountblog.com', 'sx.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbsx', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB cm', 'pmta', 'Jamie @ Discount Blog', 'hello@cm.discountblog.com', 'reply@cm.discountblog.com', 'cm.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbcm', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB sq', 'pmta', 'Jamie @ Discount Blog', 'hello@sq.discountblog.com', 'reply@sq.discountblog.com', 'sq.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbsq', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB fp', 'pmta', 'Jamie @ Discount Blog', 'hello@fp.discountblog.com', 'reply@fp.discountblog.com', 'fp.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbfp', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB wm', 'pmta', 'Jamie @ Discount Blog', 'hello@wm.discountblog.com', 'reply@wm.discountblog.com', 'wm.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbwm', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX DB gs', 'pmta', 'Jamie @ Discount Blog', 'hello@gs.discountblog.com', 'reply@gs.discountblog.com', 'gs.discountblog.com', '15.204.101.125', 587, 't.em.discountblog.com', 1500, 12000, 'dbgs', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FC ot', 'pmta', 'Eleanor @ Financial Calculate', 'hello@ot.financialcalculate.com', 'reply@ot.financialcalculate.com', 'ot.financialcalculate.com', '15.204.101.125', 587, 't.em.financialcalculate.com', 1500, 12000, 'fcot', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FC rc', 'pmta', 'Eleanor @ Financial Calculate', 'hello@rc.financialcalculate.com', 'reply@rc.financialcalculate.com', 'rc.financialcalculate.com', '15.204.101.125', 587, 't.em.financialcalculate.com', 1500, 12000, 'fcrc', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH uz', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@uz.firsttimebuyerhomeloan.com', 'reply@uz.firsttimebuyerhomeloan.com', 'uz.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthuz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH ho', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@ho.firsttimebuyerhomeloan.com', 'reply@ho.firsttimebuyerhomeloan.com', 'ho.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthho', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH ag', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@ag.firsttimebuyerhomeloan.com', 'reply@ag.firsttimebuyerhomeloan.com', 'ag.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthag', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH nw', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@nw.firsttimebuyerhomeloan.com', 'reply@nw.firsttimebuyerhomeloan.com', 'nw.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthnw', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH nd', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@nd.firsttimebuyerhomeloan.com', 'reply@nd.firsttimebuyerhomeloan.com', 'nd.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthnd', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH to', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@to.firsttimebuyerhomeloan.com', 'reply@to.firsttimebuyerhomeloan.com', 'to.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthto', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH px', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@px.firsttimebuyerhomeloan.com', 'reply@px.firsttimebuyerhomeloan.com', 'px.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthpx', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH mz', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@mz.firsttimebuyerhomeloan.com', 'reply@mz.firsttimebuyerhomeloan.com', 'mz.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthmz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH gm', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@gm.firsttimebuyerhomeloan.com', 'reply@gm.firsttimebuyerhomeloan.com', 'gm.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthgm', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH ma', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@ma.firsttimebuyerhomeloan.com', 'reply@ma.firsttimebuyerhomeloan.com', 'ma.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthma', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX FTH vp', 'pmta', 'Ethan @ First Time Buyer Home Loan', 'hello@vp.firsttimebuyerhomeloan.com', 'reply@vp.firsttimebuyerhomeloan.com', 'vp.firsttimebuyerhomeloan.com', '40.160.129.116', 587, 'trk.em.firsttimebuyerhomeloan.com', 1500, 12000, 'fthvp', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HFC bl', 'pmta', 'Holly @ Home Finance & Credit Lines', 'hello@bl.hfcl.net', 'reply@bl.hfcl.net', 'bl.hfcl.net', '40.160.129.116', 587, 'trk.em.hfcl.net', 1500, 12000, 'hfcbl', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HFC ox', 'pmta', 'Holly @ Home Finance & Credit Lines', 'hello@ox.hfcl.net', 'reply@ox.hfcl.net', 'ox.hfcl.net', '40.160.129.116', 587, 'trk.em.hfcl.net', 1500, 12000, 'hfcox', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ zk', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@zk.homeloansbyjaime.com', 'reply@zk.homeloansbyjaime.com', 'zk.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljzk', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ fr', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@fr.homeloansbyjaime.com', 'reply@fr.homeloansbyjaime.com', 'fr.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljfr', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ yh', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@yh.homeloansbyjaime.com', 'reply@yh.homeloansbyjaime.com', 'yh.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljyh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ hi', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@hi.homeloansbyjaime.com', 'reply@hi.homeloansbyjaime.com', 'hi.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljhi', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ ww', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@ww.homeloansbyjaime.com', 'reply@ww.homeloansbyjaime.com', 'ww.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljww', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ dk', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@dk.homeloansbyjaime.com', 'reply@dk.homeloansbyjaime.com', 'dk.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljdk', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ fz', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@fz.homeloansbyjaime.com', 'reply@fz.homeloansbyjaime.com', 'fz.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljfz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ vv', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@vv.homeloansbyjaime.com', 'reply@vv.homeloansbyjaime.com', 'vv.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljvv', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ zz', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@zz.homeloansbyjaime.com', 'reply@zz.homeloansbyjaime.com', 'zz.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljzz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ fk', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@fk.homeloansbyjaime.com', 'reply@fk.homeloansbyjaime.com', 'fk.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljfk', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HLJ qk', 'pmta', 'Jaime @ Home Loans by Jaime', 'hello@qk.homeloansbyjaime.com', 'reply@qk.homeloansbyjaime.com', 'qk.homeloansbyjaime.com', '40.160.129.116', 587, 'trk.em.homeloansbyjaime.com', 1500, 12000, 'hljqk', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT jk', 'pmta', 'History Thinking', 'hello@jk.historythinking.com', 'reply@jk.historythinking.com', 'jk.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htjk', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ps', 'pmta', 'History Thinking', 'hello@ps.historythinking.com', 'reply@ps.historythinking.com', 'ps.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htps', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ux', 'pmta', 'History Thinking', 'hello@ux.historythinking.com', 'reply@ux.historythinking.com', 'ux.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htux', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ck', 'pmta', 'History Thinking', 'hello@ck.historythinking.com', 'reply@ck.historythinking.com', 'ck.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htck', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT qn', 'pmta', 'History Thinking', 'hello@qn.historythinking.com', 'reply@qn.historythinking.com', 'qn.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htqn', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT fi', 'pmta', 'History Thinking', 'hello@fi.historythinking.com', 'reply@fi.historythinking.com', 'fi.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htfi', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT bb', 'pmta', 'History Thinking', 'hello@bb.historythinking.com', 'reply@bb.historythinking.com', 'bb.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htbb', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT at', 'pmta', 'History Thinking', 'hello@at.historythinking.com', 'reply@at.historythinking.com', 'at.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htat', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT na', 'pmta', 'History Thinking', 'hello@na.historythinking.com', 'reply@na.historythinking.com', 'na.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htna', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT we', 'pmta', 'History Thinking', 'hello@we.historythinking.com', 'reply@we.historythinking.com', 'we.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htwe', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT hw', 'pmta', 'History Thinking', 'hello@hw.historythinking.com', 'reply@hw.historythinking.com', 'hw.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'hthw', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ub', 'pmta', 'History Thinking', 'hello@ub.historythinking.com', 'reply@ub.historythinking.com', 'ub.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htub', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ye', 'pmta', 'History Thinking', 'hello@ye.historythinking.com', 'reply@ye.historythinking.com', 'ye.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htye', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT mu', 'pmta', 'History Thinking', 'hello@mu.historythinking.com', 'reply@mu.historythinking.com', 'mu.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htmu', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ij', 'pmta', 'History Thinking', 'hello@ij.historythinking.com', 'reply@ij.historythinking.com', 'ij.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htij', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ib', 'pmta', 'History Thinking', 'hello@ib.historythinking.com', 'reply@ib.historythinking.com', 'ib.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htib', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT yu', 'pmta', 'History Thinking', 'hello@yu.historythinking.com', 'reply@yu.historythinking.com', 'yu.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htyu', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT wc', 'pmta', 'History Thinking', 'hello@wc.historythinking.com', 'reply@wc.historythinking.com', 'wc.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htwc', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT jj', 'pmta', 'History Thinking', 'hello@jj.historythinking.com', 'reply@jj.historythinking.com', 'jj.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htjj', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ly', 'pmta', 'History Thinking', 'hello@ly.historythinking.com', 'reply@ly.historythinking.com', 'ly.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htly', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT vr', 'pmta', 'History Thinking', 'hello@vr.historythinking.com', 'reply@vr.historythinking.com', 'vr.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htvr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HT ph', 'pmta', 'History Thinking', 'hello@ph.historythinking.com', 'reply@ph.historythinking.com', 'ph.historythinking.com', '15.204.107.107', 587, 'trk.em.historythinking.com', 1500, 12000, 'htph', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM nz', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@nz.hometracmortgage.com', 'reply@nz.hometracmortgage.com', 'nz.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmnz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM wb', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@wb.hometracmortgage.com', 'reply@wb.hometracmortgage.com', 'wb.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmwb', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM hj', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@hj.hometracmortgage.com', 'reply@hj.hometracmortgage.com', 'hj.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmhj', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM hh', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@hh.hometracmortgage.com', 'reply@hh.hometracmortgage.com', 'hh.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmhh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM ju', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@ju.hometracmortgage.com', 'reply@ju.hometracmortgage.com', 'ju.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmju', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM dv', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@dv.hometracmortgage.com', 'reply@dv.hometracmortgage.com', 'dv.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmdv', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM pq', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@pq.hometracmortgage.com', 'reply@pq.hometracmortgage.com', 'pq.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmpq', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM nq', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@nq.hometracmortgage.com', 'reply@nq.hometracmortgage.com', 'nq.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmnq', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM ld', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@ld.hometracmortgage.com', 'reply@ld.hometracmortgage.com', 'ld.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmld', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM bw', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@bw.hometracmortgage.com', 'reply@bw.hometracmortgage.com', 'bw.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmbw', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HTM ik', 'pmta', 'Henry @ HomeTrac Mortgage', 'hello@ik.hometracmortgage.com', 'reply@ik.hometracmortgage.com', 'ik.hometracmortgage.com', '40.160.129.116', 587, 'trk.em.hometracmortgage.com', 1500, 12000, 'htmik', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX HW ed', 'pmta', 'Hank @ Home Warranty Services', 'hello@ed.homewarrantyservices.org', 'reply@ed.homewarrantyservices.org', 'ed.homewarrantyservices.org', '15.204.107.107', 587, 't.em.homewarrantyservices.org', 1500, 12000, 'hwed', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX LP if', 'pmta', 'Linda @ Learn Personal Loans', 'hello@if.learnpersonalloans.com', 'reply@if.learnpersonalloans.com', 'if.learnpersonalloans.com', '15.204.107.107', 587, 't.em.learnpersonalloans.com', 1500, 12000, 'lpif', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX LP ug', 'pmta', 'Linda @ Learn Personal Loans', 'hello@ug.learnpersonalloans.com', 'reply@ug.learnpersonalloans.com', 'ug.learnpersonalloans.com', '15.204.107.107', 587, 't.em.learnpersonalloans.com', 1500, 12000, 'lpug', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH tu', 'pmta', 'Arnold @ My Own Health', 'hello@tu.myownhealth.net', 'reply@tu.myownhealth.net', 'tu.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhtu', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH aw', 'pmta', 'Arnold @ My Own Health', 'hello@aw.myownhealth.net', 'reply@aw.myownhealth.net', 'aw.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhaw', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH uv', 'pmta', 'Arnold @ My Own Health', 'hello@uv.myownhealth.net', 'reply@uv.myownhealth.net', 'uv.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhuv', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH nl', 'pmta', 'Arnold @ My Own Health', 'hello@nl.myownhealth.net', 'reply@nl.myownhealth.net', 'nl.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhnl', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH kx', 'pmta', 'Arnold @ My Own Health', 'hello@kx.myownhealth.net', 'reply@kx.myownhealth.net', 'kx.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhkx', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH lr', 'pmta', 'Arnold @ My Own Health', 'hello@lr.myownhealth.net', 'reply@lr.myownhealth.net', 'lr.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhlr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH nk', 'pmta', 'Arnold @ My Own Health', 'hello@nk.myownhealth.net', 'reply@nk.myownhealth.net', 'nk.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhnk', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH fe', 'pmta', 'Arnold @ My Own Health', 'hello@fe.myownhealth.net', 'reply@fe.myownhealth.net', 'fe.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhfe', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH ro', 'pmta', 'Arnold @ My Own Health', 'hello@ro.myownhealth.net', 'reply@ro.myownhealth.net', 'ro.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhro', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH tc', 'pmta', 'Arnold @ My Own Health', 'hello@tc.myownhealth.net', 'reply@tc.myownhealth.net', 'tc.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhtc', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH ba', 'pmta', 'Arnold @ My Own Health', 'hello@ba.myownhealth.net', 'reply@ba.myownhealth.net', 'ba.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhba', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH br', 'pmta', 'Arnold @ My Own Health', 'hello@br.myownhealth.net', 'reply@br.myownhealth.net', 'br.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhbr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH pd', 'pmta', 'Arnold @ My Own Health', 'hello@pd.myownhealth.net', 'reply@pd.myownhealth.net', 'pd.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhpd', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH ex', 'pmta', 'Arnold @ My Own Health', 'hello@ex.myownhealth.net', 'reply@ex.myownhealth.net', 'ex.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhex', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH ol', 'pmta', 'Arnold @ My Own Health', 'hello@ol.myownhealth.net', 'reply@ol.myownhealth.net', 'ol.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhol', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH ko', 'pmta', 'Arnold @ My Own Health', 'hello@ko.myownhealth.net', 'reply@ko.myownhealth.net', 'ko.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhko', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH pc', 'pmta', 'Arnold @ My Own Health', 'hello@pc.myownhealth.net', 'reply@pc.myownhealth.net', 'pc.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhpc', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH zu', 'pmta', 'Arnold @ My Own Health', 'hello@zu.myownhealth.net', 'reply@zu.myownhealth.net', 'zu.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhzu', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH au', 'pmta', 'Arnold @ My Own Health', 'hello@au.myownhealth.net', 'reply@au.myownhealth.net', 'au.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhau', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH qt', 'pmta', 'Arnold @ My Own Health', 'hello@qt.myownhealth.net', 'reply@qt.myownhealth.net', 'qt.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhqt', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH wg', 'pmta', 'Arnold @ My Own Health', 'hello@wg.myownhealth.net', 'reply@wg.myownhealth.net', 'wg.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhwg', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH tr', 'pmta', 'Arnold @ My Own Health', 'hello@tr.myownhealth.net', 'reply@tr.myownhealth.net', 'tr.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhtr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH fw', 'pmta', 'Arnold @ My Own Health', 'hello@fw.myownhealth.net', 'reply@fw.myownhealth.net', 'fw.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhfw', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH ix', 'pmta', 'Arnold @ My Own Health', 'hello@ix.myownhealth.net', 'reply@ix.myownhealth.net', 'ix.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhix', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MH lg', 'pmta', 'Arnold @ My Own Health', 'hello@lg.myownhealth.net', 'reply@lg.myownhealth.net', 'lg.myownhealth.net', '15.204.107.107', 587, 'trk.em.myownhealth.net', 1500, 12000, 'mhlg', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP wj', 'pmta', 'My Personal Financial', 'hello@wj.mypersonalfinancial.com', 'reply@wj.mypersonalfinancial.com', 'wj.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfwj', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP jl', 'pmta', 'My Personal Financial', 'hello@jl.mypersonalfinancial.com', 'reply@jl.mypersonalfinancial.com', 'jl.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfjl', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP ee', 'pmta', 'My Personal Financial', 'hello@ee.mypersonalfinancial.com', 'reply@ee.mypersonalfinancial.com', 'ee.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfee', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP ws', 'pmta', 'My Personal Financial', 'hello@ws.mypersonalfinancial.com', 'reply@ws.mypersonalfinancial.com', 'ws.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfws', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP kq', 'pmta', 'My Personal Financial', 'hello@kq.mypersonalfinancial.com', 'reply@kq.mypersonalfinancial.com', 'kq.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfkq', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP xr', 'pmta', 'My Personal Financial', 'hello@xr.mypersonalfinancial.com', 'reply@xr.mypersonalfinancial.com', 'xr.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfxr', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP wo', 'pmta', 'My Personal Financial', 'hello@wo.mypersonalfinancial.com', 'reply@wo.mypersonalfinancial.com', 'wo.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfwo', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP ep', 'pmta', 'My Personal Financial', 'hello@ep.mypersonalfinancial.com', 'reply@ep.mypersonalfinancial.com', 'ep.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfep', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP ou', 'pmta', 'My Personal Financial', 'hello@ou.mypersonalfinancial.com', 'reply@ou.mypersonalfinancial.com', 'ou.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfou', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP yn', 'pmta', 'My Personal Financial', 'hello@yn.mypersonalfinancial.com', 'reply@yn.mypersonalfinancial.com', 'yn.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfyn', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MP ec', 'pmta', 'My Personal Financial', 'hello@ec.mypersonalfinancial.com', 'reply@ec.mypersonalfinancial.com', 'ec.mypersonalfinancial.com', '40.160.129.116', 587, 'trk.em.mypersonalfinancial.com', 1500, 12000, 'mpfec', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MR vi', 'pmta', 'Bob @ My Repair DIY', 'hello@vi.myrepairdiy.com', 'reply@vi.myrepairdiy.com', 'vi.myrepairdiy.com', '15.204.101.125', 587, 't.em.myrepairdiy.com', 1500, 12000, 'mrvi', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX MR dg', 'pmta', 'Bob @ My Repair DIY', 'hello@dg.myrepairdiy.com', 'reply@dg.myrepairdiy.com', 'dg.myrepairdiy.com', '15.204.101.125', 587, 't.em.myrepairdiy.com', 1500, 12000, 'mrdg', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD py', 'pmta', 'PayMyDebit', 'hello@py.paymydebit.com', 'reply@py.paymydebit.com', 'py.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdpy', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD zv', 'pmta', 'PayMyDebit', 'hello@zv.paymydebit.com', 'reply@zv.paymydebit.com', 'zv.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdzv', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD lh', 'pmta', 'PayMyDebit', 'hello@lh.paymydebit.com', 'reply@lh.paymydebit.com', 'lh.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdlh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD oh', 'pmta', 'PayMyDebit', 'hello@oh.paymydebit.com', 'reply@oh.paymydebit.com', 'oh.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdoh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD hm', 'pmta', 'PayMyDebit', 'hello@hm.paymydebit.com', 'reply@hm.paymydebit.com', 'hm.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdhm', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD vj', 'pmta', 'PayMyDebit', 'hello@vj.paymydebit.com', 'reply@vj.paymydebit.com', 'vj.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdvj', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD ab', 'pmta', 'PayMyDebit', 'hello@ab.paymydebit.com', 'reply@ab.paymydebit.com', 'ab.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdab', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD xn', 'pmta', 'PayMyDebit', 'hello@xn.paymydebit.com', 'reply@xn.paymydebit.com', 'xn.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdxn', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD sh', 'pmta', 'PayMyDebit', 'hello@sh.paymydebit.com', 'reply@sh.paymydebit.com', 'sh.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdsh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD of', 'pmta', 'PayMyDebit', 'hello@of.paymydebit.com', 'reply@of.paymydebit.com', 'of.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdof', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX PD cv', 'pmta', 'PayMyDebit', 'hello@cv.paymydebit.com', 'reply@cv.paymydebit.com', 'cv.paymydebit.com', '40.160.129.116', 587, 'trk.em.paymydebit.com', 1500, 12000, 'pdcv', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF be', 'pmta', 'Quiz Master', 'hello@be.quizfiesta.com', 'reply@be.quizfiesta.com', 'be.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfbe', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF cs', 'pmta', 'Quiz Master', 'hello@cs.quizfiesta.com', 'reply@cs.quizfiesta.com', 'cs.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfcs', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF af', 'pmta', 'Quiz Master', 'hello@af.quizfiesta.com', 'reply@af.quizfiesta.com', 'af.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfaf', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF ua', 'pmta', 'Quiz Master', 'hello@ua.quizfiesta.com', 'reply@ua.quizfiesta.com', 'ua.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfua', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF ta', 'pmta', 'Quiz Master', 'hello@ta.quizfiesta.com', 'reply@ta.quizfiesta.com', 'ta.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfta', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF qj', 'pmta', 'Quiz Master', 'hello@qj.quizfiesta.com', 'reply@qj.quizfiesta.com', 'qj.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfqj', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF bs', 'pmta', 'Quiz Master', 'hello@bs.quizfiesta.com', 'reply@bs.quizfiesta.com', 'bs.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfbs', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF jw', 'pmta', 'Quiz Master', 'hello@jw.quizfiesta.com', 'reply@jw.quizfiesta.com', 'jw.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfjw', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF dl', 'pmta', 'Quiz Master', 'hello@dl.quizfiesta.com', 'reply@dl.quizfiesta.com', 'dl.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfdl', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF dm', 'pmta', 'Quiz Master', 'hello@dm.quizfiesta.com', 'reply@dm.quizfiesta.com', 'dm.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfdm', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF dt', 'pmta', 'Quiz Master', 'hello@dt.quizfiesta.com', 'reply@dt.quizfiesta.com', 'dt.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfdt', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF rv', 'pmta', 'Quiz Master', 'hello@rv.quizfiesta.com', 'reply@rv.quizfiesta.com', 'rv.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfrv', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF yp', 'pmta', 'Quiz Master', 'hello@yp.quizfiesta.com', 'reply@yp.quizfiesta.com', 'yp.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfyp', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF ya', 'pmta', 'Quiz Master', 'hello@ya.quizfiesta.com', 'reply@ya.quizfiesta.com', 'ya.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfya', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF ad', 'pmta', 'Quiz Master', 'hello@ad.quizfiesta.com', 'reply@ad.quizfiesta.com', 'ad.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfad', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF wt', 'pmta', 'Quiz Master', 'hello@wt.quizfiesta.com', 'reply@wt.quizfiesta.com', 'wt.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfwt', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF rm', 'pmta', 'Quiz Master', 'hello@rm.quizfiesta.com', 'reply@rm.quizfiesta.com', 'rm.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfrm', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF av', 'pmta', 'Quiz Master', 'hello@av.quizfiesta.com', 'reply@av.quizfiesta.com', 'av.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfav', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF qg', 'pmta', 'Quiz Master', 'hello@qg.quizfiesta.com', 'reply@qg.quizfiesta.com', 'qg.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfqg', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF lo', 'pmta', 'Quiz Master', 'hello@lo.quizfiesta.com', 'reply@lo.quizfiesta.com', 'lo.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qflo', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF mb', 'pmta', 'Quiz Master', 'hello@mb.quizfiesta.com', 'reply@mb.quizfiesta.com', 'mb.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfmb', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF zt', 'pmta', 'Quiz Master', 'hello@zt.quizfiesta.com', 'reply@zt.quizfiesta.com', 'zt.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfzt', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF si', 'pmta', 'Quiz Master', 'hello@si.quizfiesta.com', 'reply@si.quizfiesta.com', 'si.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfsi', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF ob', 'pmta', 'Quiz Master', 'hello@ob.quizfiesta.com', 'reply@ob.quizfiesta.com', 'ob.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfob', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX QF gd', 'pmta', 'Quiz Master', 'hello@gd.quizfiesta.com', 'reply@gd.quizfiesta.com', 'gd.quizfiesta.com', '15.204.101.125', 587, 't.em.quizfiesta.com', 1500, 12000, 'qfgd', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX RB hp', 'pmta', 'Sam @ Rates Bazar', 'hello@hp.ratesbazar.com', 'reply@hp.ratesbazar.com', 'hp.ratesbazar.com', '15.204.107.107', 587, 't.em.ratesbazar.com', 1500, 12000, 'rbhp', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX RB az', 'pmta', 'Sam @ Rates Bazar', 'hello@az.ratesbazar.com', 'reply@az.ratesbazar.com', 'az.ratesbazar.com', '15.204.107.107', 587, 't.em.ratesbazar.com', 1500, 12000, 'rbaz', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX RR uc', 'pmta', 'Frank @ Refinance Rates USA', 'hello@uc.refinanceratesusa.com', 'reply@uc.refinanceratesusa.com', 'uc.refinanceratesusa.com', '15.204.107.107', 587, 't.em.refinanceratesusa.com', 1500, 12000, 'rruc', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX RR go', 'pmta', 'Frank @ Refinance Rates USA', 'hello@go.refinanceratesusa.com', 'reply@go.refinanceratesusa.com', 'go.refinanceratesusa.com', '15.204.107.107', 587, 't.em.refinanceratesusa.com', 1500, 12000, 'rrgo', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR gz', 'pmta', 'The Retirement Blog', 'hello@gz.theretirementblog.com', 'reply@gz.theretirementblog.com', 'gz.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trgz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR oi', 'pmta', 'The Retirement Blog', 'hello@oi.theretirementblog.com', 'reply@oi.theretirementblog.com', 'oi.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'troi', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR vn', 'pmta', 'The Retirement Blog', 'hello@vn.theretirementblog.com', 'reply@vn.theretirementblog.com', 'vn.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trvn', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR ks', 'pmta', 'The Retirement Blog', 'hello@ks.theretirementblog.com', 'reply@ks.theretirementblog.com', 'ks.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trks', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR kh', 'pmta', 'The Retirement Blog', 'hello@kh.theretirementblog.com', 'reply@kh.theretirementblog.com', 'kh.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trkh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR lz', 'pmta', 'The Retirement Blog', 'hello@lz.theretirementblog.com', 'reply@lz.theretirementblog.com', 'lz.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trlz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR hc', 'pmta', 'The Retirement Blog', 'hello@hc.theretirementblog.com', 'reply@hc.theretirementblog.com', 'hc.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trhc', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR ow', 'pmta', 'The Retirement Blog', 'hello@ow.theretirementblog.com', 'reply@ow.theretirementblog.com', 'ow.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trow', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR yb', 'pmta', 'The Retirement Blog', 'hello@yb.theretirementblog.com', 'reply@yb.theretirementblog.com', 'yb.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'tryb', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR dr', 'pmta', 'The Retirement Blog', 'hello@dr.theretirementblog.com', 'reply@dr.theretirementblog.com', 'dr.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trdr', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TR dn', 'pmta', 'The Retirement Blog', 'hello@dn.theretirementblog.com', 'reply@dn.theretirementblog.com', 'dn.theretirementblog.com', '40.160.129.116', 587, 'trk.em.theretirementblog.com', 1500, 12000, 'trdn', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TT qb', 'pmta', 'Olivia @ Thing of the Day', 'hello@qb.thingoftheday.org', 'reply@qb.thingoftheday.org', 'qb.thingoftheday.org', '15.204.107.107', 587, 't.em.thingoftheday.org', 1500, 12000, 'ttqb', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX TT sr', 'pmta', 'Olivia @ Thing of the Day', 'hello@sr.thingoftheday.org', 'reply@sr.thingoftheday.org', 'sr.thingoftheday.org', '15.204.107.107', 587, 't.em.thingoftheday.org', 1500, 12000, 'ttsr', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF xv', 'pmta', 'Walter @ US Finance', 'hello@xv.us-finance.com', 'reply@xv.us-finance.com', 'xv.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfxv', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF ge', 'pmta', 'Walter @ US Finance', 'hello@ge.us-finance.com', 'reply@ge.us-finance.com', 'ge.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfge', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF tx', 'pmta', 'Walter @ US Finance', 'hello@tx.us-finance.com', 'reply@tx.us-finance.com', 'tx.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usftx', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF zh', 'pmta', 'Walter @ US Finance', 'hello@zh.us-finance.com', 'reply@zh.us-finance.com', 'zh.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfzh', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF is', 'pmta', 'Walter @ US Finance', 'hello@is.us-finance.com', 'reply@is.us-finance.com', 'is.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfis', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF en', 'pmta', 'Walter @ US Finance', 'hello@en.us-finance.com', 'reply@en.us-finance.com', 'en.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfen', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF vg', 'pmta', 'Walter @ US Finance', 'hello@vg.us-finance.com', 'reply@vg.us-finance.com', 'vg.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfvg', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF rx', 'pmta', 'Walter @ US Finance', 'hello@rx.us-finance.com', 'reply@rx.us-finance.com', 'rx.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfrx', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF fq', 'pmta', 'Walter @ US Finance', 'hello@fq.us-finance.com', 'reply@fq.us-finance.com', 'fq.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usffq', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF cy', 'pmta', 'Walter @ US Finance', 'hello@cy.us-finance.com', 'reply@cy.us-finance.com', 'cy.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfcy', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX USF hz', 'pmta', 'Walter @ US Finance', 'hello@hz.us-finance.com', 'reply@hz.us-finance.com', 'hz.us-finance.com', '40.160.129.116', 587, 'trk.em.us-finance.com', 1500, 12000, 'usfhz', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX WF ay', 'pmta', 'Greg @ Warranty For You', 'hello@ay.warrantyforyou.com', 'reply@ay.warrantyforyou.com', 'ay.warrantyforyou.com', '15.204.107.107', 587, 't.em.warrantyforyou.com', 1500, 12000, 'wfay', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX WF ef', 'pmta', 'Greg @ Warranty For You', 'hello@ef.warrantyforyou.com', 'reply@ef.warrantyforyou.com', 'ef.warrantyforyou.com', '15.204.107.107', 587, 't.em.warrantyforyou.com', 1500, 12000, 'wfef', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB ud', 'pmta', 'Claire @ Your Financial Blog', 'hello@ud.yourfinancialblog.com', 'reply@ud.yourfinancialblog.com', 'ud.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbud', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB nm', 'pmta', 'Claire @ Your Financial Blog', 'hello@nm.yourfinancialblog.com', 'reply@nm.yourfinancialblog.com', 'nm.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbnm', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB uw', 'pmta', 'Claire @ Your Financial Blog', 'hello@uw.yourfinancialblog.com', 'reply@uw.yourfinancialblog.com', 'uw.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbuw', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB vs', 'pmta', 'Claire @ Your Financial Blog', 'hello@vs.yourfinancialblog.com', 'reply@vs.yourfinancialblog.com', 'vs.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbvs', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB jn', 'pmta', 'Claire @ Your Financial Blog', 'hello@jn.yourfinancialblog.com', 'reply@jn.yourfinancialblog.com', 'jn.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbjn', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB eb', 'pmta', 'Claire @ Your Financial Blog', 'hello@eb.yourfinancialblog.com', 'reply@eb.yourfinancialblog.com', 'eb.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbeb', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB bx', 'pmta', 'Claire @ Your Financial Blog', 'hello@bx.yourfinancialblog.com', 'reply@bx.yourfinancialblog.com', 'bx.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbbx', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB zw', 'pmta', 'Claire @ Your Financial Blog', 'hello@zw.yourfinancialblog.com', 'reply@zw.yourfinancialblog.com', 'zw.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbzw', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB gb', 'pmta', 'Claire @ Your Financial Blog', 'hello@gb.yourfinancialblog.com', 'reply@gb.yourfinancialblog.com', 'gb.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbgb', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB ej', 'pmta', 'Claire @ Your Financial Blog', 'hello@ej.yourfinancialblog.com', 'reply@ej.yourfinancialblog.com', 'ej.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbej', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YFB or', 'pmta', 'Claire @ Your Financial Blog', 'hello@or.yourfinancialblog.com', 'reply@or.yourfinancialblog.com', 'or.yourfinancialblog.com', '40.160.129.116', 587, 'trk.em.yourfinancialblog.com', 1500, 12000, 'yfbor', 'kumo', false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YI pl', 'pmta', 'Carl @ Your Insurance Hub', 'hello@pl.yourinsurancehub.com', 'reply@pl.yourinsurancehub.com', 'pl.yourinsurancehub.com', '15.204.107.107', 587, 't.em.yourinsurancehub.com', 1500, 12000, 'yipl', NULL, false, 'draft', false),
    ('00000000-0000-0000-0000-000000000001', 'NX YI ut', 'pmta', 'Carl @ Your Insurance Hub', 'hello@ut.yourinsurancehub.com', 'reply@ut.yourinsurancehub.com', 'ut.yourinsurancehub.com', '15.204.107.107', 587, 't.em.yourinsurancehub.com', 1500, 12000, 'yiut', NULL, false, 'draft', false)
) AS v(org, name, vendor, from_name, from_email, reply_email, sending_domain,
       smtp_host, smtp_port, tracking_domain, hourly_limit, daily_limit,
       pool_prefix, routing_mode, via_ses, status, is_default)
WHERE NOT EXISTS (
    SELECT 1 FROM mailing_sending_profiles p
    WHERE p.sending_domain = v.sending_domain AND p.organization_id = v.org::uuid);
`},

		// nx wave pools + address repoint. Order matters: the pools are created
		// first, and the repoint UPDATE joins them by name. The repoint selects by
		// an EXPLICIT (ip, pool_name) list captured from the plan, never by the
		// value under change. None of the 425 addresses is one of the 60 the
		// aug26_yahoo_set_ip_reassert migration rewrites on every boot (asserted in
		// agents/jobs/nx_pool_seed.py, which refuses to emit on a collision).
		{"nx_wave_ip_pools_20260918", `
INSERT INTO mailing_ip_pools (id, organization_id, name, description, pool_type, status, created_at, updated_at)
SELECT gen_random_uuid(), v.org::uuid, v.name, 'nx wave ' || v.dom, 'general', 'active', NOW(), NOW()
FROM (VALUES

    ('00000000-0000-0000-0000-000000000001', 'aadpg-general-pool', 'pg.aadwd.com'),
    ('00000000-0000-0000-0000-000000000001', 'aadls-general-pool', 'ls.aadwd.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccrp-general-pool', 'rp.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccgk-general-pool', 'gk.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccru-general-pool', 'ru.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccpz-general-pool', 'pz.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccrk-general-pool', 'rk.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccct-general-pool', 'ct.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccxa-general-pool', 'xa.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccax-general-pool', 'ax.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccgp-general-pool', 'gp.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccrs-general-pool', 'rs.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bccon-general-pool', 'on.bestcreditcare.com'),
    ('00000000-0000-0000-0000-000000000001', 'bwus-general-pool', 'us.businessweeklypro.com'),
    ('00000000-0000-0000-0000-000000000001', 'bwyr-general-pool', 'yr.businessweeklypro.com'),
    ('00000000-0000-0000-0000-000000000001', 'cioj-general-pool', 'oj.casainsure.com'),
    ('00000000-0000-0000-0000-000000000001', 'cigu-general-pool', 'gu.casainsure.com'),
    ('00000000-0000-0000-0000-000000000001', 'cpur-general-pool', 'ur.consumerpro.net'),
    ('00000000-0000-0000-0000-000000000001', 'cppp-general-pool', 'pp.consumerpro.net'),
    ('00000000-0000-0000-0000-000000000001', 'dbpf-general-pool', 'pf.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbgr-general-pool', 'gr.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbil-general-pool', 'il.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbal-general-pool', 'al.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbae-general-pool', 'ae.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbam-general-pool', 'am.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbqi-general-pool', 'qi.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbiy-general-pool', 'iy.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbaa-general-pool', 'aa.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbjt-general-pool', 'jt.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbdc-general-pool', 'dc.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbwn-general-pool', 'wn.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbnn-general-pool', 'nn.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbdx-general-pool', 'dx.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbmh-general-pool', 'mh.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbkm-general-pool', 'km.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbhk-general-pool', 'hk.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbez-general-pool', 'ez.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbjy-general-pool', 'jy.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbsx-general-pool', 'sx.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbcm-general-pool', 'cm.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbsq-general-pool', 'sq.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbfp-general-pool', 'fp.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbwm-general-pool', 'wm.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'dbgs-general-pool', 'gs.discountblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'fcot-general-pool', 'ot.financialcalculate.com'),
    ('00000000-0000-0000-0000-000000000001', 'fcrc-general-pool', 'rc.financialcalculate.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthuz-general-pool', 'uz.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthho-general-pool', 'ho.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthag-general-pool', 'ag.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthnw-general-pool', 'nw.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthnd-general-pool', 'nd.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthto-general-pool', 'to.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthpx-general-pool', 'px.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthmz-general-pool', 'mz.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthgm-general-pool', 'gm.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthma-general-pool', 'ma.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'fthvp-general-pool', 'vp.firsttimebuyerhomeloan.com'),
    ('00000000-0000-0000-0000-000000000001', 'hfcbl-general-pool', 'bl.hfcl.net'),
    ('00000000-0000-0000-0000-000000000001', 'hfcox-general-pool', 'ox.hfcl.net'),
    ('00000000-0000-0000-0000-000000000001', 'hljzk-general-pool', 'zk.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljfr-general-pool', 'fr.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljyh-general-pool', 'yh.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljhi-general-pool', 'hi.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljww-general-pool', 'ww.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljdk-general-pool', 'dk.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljfz-general-pool', 'fz.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljvv-general-pool', 'vv.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljzz-general-pool', 'zz.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljfk-general-pool', 'fk.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'hljqk-general-pool', 'qk.homeloansbyjaime.com'),
    ('00000000-0000-0000-0000-000000000001', 'htjk-general-pool', 'jk.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htps-general-pool', 'ps.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htux-general-pool', 'ux.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htck-general-pool', 'ck.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htqn-general-pool', 'qn.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htfi-general-pool', 'fi.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htbb-general-pool', 'bb.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htat-general-pool', 'at.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htna-general-pool', 'na.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htwe-general-pool', 'we.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'hthw-general-pool', 'hw.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htub-general-pool', 'ub.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htye-general-pool', 'ye.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmu-general-pool', 'mu.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htij-general-pool', 'ij.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htib-general-pool', 'ib.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htyu-general-pool', 'yu.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htwc-general-pool', 'wc.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htjj-general-pool', 'jj.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htly-general-pool', 'ly.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htvr-general-pool', 'vr.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htph-general-pool', 'ph.historythinking.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmnz-general-pool', 'nz.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmwb-general-pool', 'wb.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmhj-general-pool', 'hj.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmhh-general-pool', 'hh.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmju-general-pool', 'ju.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmdv-general-pool', 'dv.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmpq-general-pool', 'pq.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmnq-general-pool', 'nq.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmld-general-pool', 'ld.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmbw-general-pool', 'bw.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'htmik-general-pool', 'ik.hometracmortgage.com'),
    ('00000000-0000-0000-0000-000000000001', 'hwed-general-pool', 'ed.homewarrantyservices.org'),
    ('00000000-0000-0000-0000-000000000001', 'lpif-general-pool', 'if.learnpersonalloans.com'),
    ('00000000-0000-0000-0000-000000000001', 'lpug-general-pool', 'ug.learnpersonalloans.com'),
    ('00000000-0000-0000-0000-000000000001', 'mhtu-general-pool', 'tu.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhaw-general-pool', 'aw.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhuv-general-pool', 'uv.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhnl-general-pool', 'nl.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhkx-general-pool', 'kx.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhlr-general-pool', 'lr.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhnk-general-pool', 'nk.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhfe-general-pool', 'fe.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhro-general-pool', 'ro.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhtc-general-pool', 'tc.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhba-general-pool', 'ba.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhbr-general-pool', 'br.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhpd-general-pool', 'pd.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhex-general-pool', 'ex.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhol-general-pool', 'ol.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhko-general-pool', 'ko.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhpc-general-pool', 'pc.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhzu-general-pool', 'zu.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhau-general-pool', 'au.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhqt-general-pool', 'qt.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhwg-general-pool', 'wg.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhtr-general-pool', 'tr.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhfw-general-pool', 'fw.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhix-general-pool', 'ix.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mhlg-general-pool', 'lg.myownhealth.net'),
    ('00000000-0000-0000-0000-000000000001', 'mpfwj-general-pool', 'wj.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfjl-general-pool', 'jl.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfee-general-pool', 'ee.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfws-general-pool', 'ws.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfkq-general-pool', 'kq.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfxr-general-pool', 'xr.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfwo-general-pool', 'wo.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfep-general-pool', 'ep.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfou-general-pool', 'ou.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfyn-general-pool', 'yn.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mpfec-general-pool', 'ec.mypersonalfinancial.com'),
    ('00000000-0000-0000-0000-000000000001', 'mrvi-general-pool', 'vi.myrepairdiy.com'),
    ('00000000-0000-0000-0000-000000000001', 'mrdg-general-pool', 'dg.myrepairdiy.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdpy-general-pool', 'py.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdzv-general-pool', 'zv.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdlh-general-pool', 'lh.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdoh-general-pool', 'oh.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdhm-general-pool', 'hm.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdvj-general-pool', 'vj.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdab-general-pool', 'ab.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdxn-general-pool', 'xn.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdsh-general-pool', 'sh.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdof-general-pool', 'of.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'pdcv-general-pool', 'cv.paymydebit.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfbe-general-pool', 'be.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfcs-general-pool', 'cs.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfaf-general-pool', 'af.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfua-general-pool', 'ua.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfta-general-pool', 'ta.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfqj-general-pool', 'qj.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfbs-general-pool', 'bs.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfjw-general-pool', 'jw.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfdl-general-pool', 'dl.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfdm-general-pool', 'dm.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfdt-general-pool', 'dt.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfrv-general-pool', 'rv.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfyp-general-pool', 'yp.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfya-general-pool', 'ya.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfad-general-pool', 'ad.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfwt-general-pool', 'wt.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfrm-general-pool', 'rm.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfav-general-pool', 'av.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfqg-general-pool', 'qg.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qflo-general-pool', 'lo.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfmb-general-pool', 'mb.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfzt-general-pool', 'zt.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfsi-general-pool', 'si.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfob-general-pool', 'ob.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'qfgd-general-pool', 'gd.quizfiesta.com'),
    ('00000000-0000-0000-0000-000000000001', 'rbhp-general-pool', 'hp.ratesbazar.com'),
    ('00000000-0000-0000-0000-000000000001', 'rbaz-general-pool', 'az.ratesbazar.com'),
    ('00000000-0000-0000-0000-000000000001', 'rruc-general-pool', 'uc.refinanceratesusa.com'),
    ('00000000-0000-0000-0000-000000000001', 'rrgo-general-pool', 'go.refinanceratesusa.com'),
    ('00000000-0000-0000-0000-000000000001', 'trgz-general-pool', 'gz.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'troi-general-pool', 'oi.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trvn-general-pool', 'vn.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trks-general-pool', 'ks.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trkh-general-pool', 'kh.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trlz-general-pool', 'lz.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trhc-general-pool', 'hc.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trow-general-pool', 'ow.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'tryb-general-pool', 'yb.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trdr-general-pool', 'dr.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'trdn-general-pool', 'dn.theretirementblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'ttqb-general-pool', 'qb.thingoftheday.org'),
    ('00000000-0000-0000-0000-000000000001', 'ttsr-general-pool', 'sr.thingoftheday.org'),
    ('00000000-0000-0000-0000-000000000001', 'usfxv-general-pool', 'xv.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfge-general-pool', 'ge.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usftx-general-pool', 'tx.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfzh-general-pool', 'zh.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfis-general-pool', 'is.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfen-general-pool', 'en.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfvg-general-pool', 'vg.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfrx-general-pool', 'rx.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usffq-general-pool', 'fq.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfcy-general-pool', 'cy.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'usfhz-general-pool', 'hz.us-finance.com'),
    ('00000000-0000-0000-0000-000000000001', 'wfay-general-pool', 'ay.warrantyforyou.com'),
    ('00000000-0000-0000-0000-000000000001', 'wfef-general-pool', 'ef.warrantyforyou.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbud-general-pool', 'ud.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbnm-general-pool', 'nm.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbuw-general-pool', 'uw.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbvs-general-pool', 'vs.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbjn-general-pool', 'jn.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbeb-general-pool', 'eb.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbbx-general-pool', 'bx.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbzw-general-pool', 'zw.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbgb-general-pool', 'gb.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbej-general-pool', 'ej.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yfbor-general-pool', 'or.yourfinancialblog.com'),
    ('00000000-0000-0000-0000-000000000001', 'yipl-general-pool', 'pl.yourinsurancehub.com'),
    ('00000000-0000-0000-0000-000000000001', 'yiut-general-pool', 'ut.yourinsurancehub.com')
) AS v(org, name, dom)
WHERE NOT EXISTS (SELECT 1 FROM mailing_ip_pools p
                  WHERE p.name = v.name AND p.organization_id = v.org::uuid);
`},
		{"nx_wave_ip_repoint_20260918", nxWaveIPRepointSQL},
...
END $$`},
		...
		// drip creatives to car-insurance leads.
		//
		// Authored 2026-08-08 as 'auto_coverage_internal' — but this block only
		// takes effect at BOOT, the build never deployed, and prod therefore
		// still read 'personal_loans' a day later, so the operator flagged the
		// same misconfiguration twice. The four statements below are now also
		// applied LIVE (identical SQL); they stay here so a rebuilt task can
		// never regress the vertical. Writing a startup migration is not the
		// same as changing production.
		//
		// It stays SEPARATE from the shared
		...
		{"aug08_partner_datasets_vertical_internal_auto_insurance", `DO $$
		BEGIN
			IF NOT EXISTS (
				SELECT 1 FROM pg_constraint
				WHERE conname = 'partner_datasets_vertical_check'
				  AND pg_get_constraintdef(oid) LIKE '%internal_auto_insurance%'
			) THEN
				ALTER TABLE partner_datasets DROP CONSTRAINT IF EXISTS partner_datasets_vertical_check;
				ALTER TABLE partner_datasets ADD CONSTRAINT partner_datasets_vertical_check
					CHECK (vertical = ANY (ARRAY['refi_heloc','personal_loans','tax_relief','remodel',
		...
		{"aug08_seed_internal_auto_insurance_creatives", `INSERT INTO partner_drip_creatives
			(vertical, brand, creative_filename, subject_line, preheader, from_name, offer_id, active, updated_by)
			SELECT 'internal_auto_insurance', brand, creative_filename, subject_line, preheader,
			       from_name, offer_id, active, 'aug08_auto_coverage_seed'
			FROM partner_drip_creatives
			WHERE vertical = 'auto_insurance'
			ON CONFLICT (vertical, brand) DO NOTHING`},
		{"aug08_seed_internal_auto_insurance_drip_state", `INSERT INTO partner_drip_state (vertical, next_brand_index)
			VALUES ('internal_auto_insurance', 0)
			ON CONFLICT (vertical) DO NOTHING`},
		...
		// inert behind DRIP_OBSERVATORY_ROLLUP_DISABLED / _LANES). The four
		// tables this block originally deferred (link_audit, hygiene,
		// cap_decisions, cap_xray) ship in the rev-4.1 completion-increment
		// block below, DDL restored in plan §5.6-§5.8b.
		...
		// ── Drip Observatory — rev-4.1 completion increment (plan §1) ───────
		// Restores the four STOP-1 tables with the plan's now-pinned DDL
		// (§5.6–§5.8b), ships the §5.11 campaign-meta TABLE (schema-before-
		// binary — the orchestrator WRITER remains HOLD-CRITICAL at D3b and is
		// NOT touched here), and widens the two already-shipped isp vocab
		// constraints per the rev-4.1 STOP-2 ruling (§5.0b). All entries
		// additive + idempotent; every table NEW and EMPTY (sub-5s).
		{"dob_widen_isp_vocab_other", dobWidenISPVocabOtherSQL},
		// Link-attribution audit facts (§5.6). status/row_kind have NO
		// defaults — rev-4.1 letter correction: the rev-3 'GREEN' default was
		// GREEN-by-default; writers must earn GREEN explicitly.
		{"dob_create_link_audit", `CREATE TABLE IF NOT EXISTS partner_drip_link_audit (
			id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
			run_id UUID NOT NULL REFERENCES partner_drip_observatory_runs(run_id),
			organization_id UUID NOT NULL,
			event_day DATE NOT NULL,                 -- basis: the clicks' event_at Denver day
			dataset_id UUID,
			campaign_id UUID NOT NULL,
			brand_code TEXT NOT NULL DEFAULT '',     -- '' when identity quarantined (row still audited)
			sending_apex TEXT NOT NULL DEFAULT '',
			url_host TEXT NOT NULL DEFAULT '',       -- '' on the coverage row
			row_kind TEXT NOT NULL CONSTRAINT dob_linkaudit_rowkind_vocab
				CHECK (row_kind IN ('host','coverage')),                       -- no default (§5.0)
			clicks INT NOT NULL DEFAULT 0 CHECK (clicks >= 0),                 -- host: audited money rows; coverage: total click rows
			red_clicks INT NOT NULL DEFAULT 0 CHECK (red_clicks >= 0),
			status TEXT NOT NULL CONSTRAINT dob_linkaudit_status_vocab
				CHECK (status IN ('GREEN','RED','NO_COVERAGE')),               -- no default: GREEN must be earned
			fail_reasons JSONB NOT NULL DEFAULT '{}'::jsonb,
			wrapper_rows INT NOT NULL DEFAULT 0 CHECK (wrapper_rows >= 0),     -- coverage accounting:
			resolved_rows INT NOT NULL DEFAULT 0 CHECK (resolved_rows >= 0),
			money_host_rows INT NOT NULL DEFAULT 0 CHECK (money_host_rows >= 0),
			audited_rows INT NOT NULL DEFAULT 0 CHECK (audited_rows >= 0),
			skipped_rows INT NOT NULL DEFAULT 0 CHECK (skipped_rows >= 0),
			parse_error_rows INT NOT NULL DEFAULT 0 CHECK (parse_error_rows >= 0),
			sub2_present INT NOT NULL DEFAULT 0 CHECK (sub2_present >= 0),
			sub2_invalid INT NOT NULL DEFAULT 0,
			sample_url_redacted TEXT NOT NULL DEFAULT '',                      -- §7.2 redacted form ONLY
			computed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			CHECK (red_clicks <= clicks),
			CHECK (sub2_invalid >= 0 AND sub2_invalid <= sub2_present)
		)`},
		{"dob_uq_link_audit", `CREATE UNIQUE INDEX IF NOT EXISTS uq_drip_link_audit
			ON partner_drip_link_audit (organization_id, event_day, campaign_id, url_host, row_kind, run_id)`},
		{"dob_idx_link_audit_day_status", `CREATE INDEX IF NOT EXISTS idx_drip_link_audit_day_status
			ON partner_drip_link_audit (event_day DESC, status)`},
		{"dob_idx_link_audit_run", `CREATE INDEX IF NOT EXISTS idx_drip_link_audit_run ON partner_drip_link_audit (run_id)`},
		// Hygiene facts (§5.7): dataset×day×population grain — NO brand
		// columns (rev-4.1 letter correction: the rev-4 "both gain
		// brand_code/sending_apex" over-reached for this table).
		{"dob_create_hygiene_daily", `CREATE TABLE IF NOT EXISTS partner_drip_hygiene_daily (
			id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
			run_id UUID NOT NULL REFERENCES partner_drip_observatory_runs(run_id),
			organization_id UUID NOT NULL,
			cohort_day DATE NOT NULL,
			dataset_id UUID NOT NULL,
			population TEXT NOT NULL CONSTRAINT dob_hygiene_population_vocab
				CHECK (population IN ('first_touch_dispatch','all_touch_events')),   -- no default
			dispatched_total INT NOT NULL DEFAULT 0 CHECK (dispatched_total >= 0),
			pcq_validated INT NOT NULL DEFAULT 0 CHECK (pcq_validated >= 0),
			eo_result_breakdown JSONB NOT NULL DEFAULT '{}'::jsonb,   -- {"<eo_result>|<status>": n}
			subs_matched INT NOT NULL DEFAULT 0 CHECK (subs_matched >= 0),
			subs_unmatched INT NOT NULL DEFAULT 0 CHECK (subs_unmatched >= 0),
			subs_verification_status JSONB NOT NULL DEFAULT '{}'::jsonb,
			subs_eo_validated INT NOT NULL DEFAULT 0 CHECK (subs_eo_validated >= 0),
			bounce_anatomy JSONB NOT NULL DEFAULT '{}'::jsonb,        -- hard/soft/reputation/'validation' separate keys, never summed
			complaints INT NOT NULL DEFAULT 0 CHECK (complaints >= 0),
			computed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			CONSTRAINT dob_hygiene_match_identity CHECK (
				population <> 'first_touch_dispatch' OR subs_matched + subs_unmatched = dispatched_total)
		)`},
		...
		// HOLD-CRITICAL); the table is inert schema.
		{"capdec_create_cap_decisions", `CREATE TABLE IF NOT EXISTS partner_drip_cap_decisions (
			id UUID NOT NULL DEFAULT gen_random_uuid(),
			wave_attempt_id UUID NOT NULL,           -- minted once per pass iteration (§10.1)
			decided_at TIMESTAMPTZ NOT NULL,         -- frozen ONCE at trace creation; shared by the attempt's rows
			pass TEXT NOT NULL CONSTRAINT capdec_pass_vocab CHECK (pass IN ('welcome','followup')),
			vertical TEXT NOT NULL,
			dataset_id UUID NOT NULL,                -- verified: both passes carry v.datasetID (§2)
			brand_code TEXT NOT NULL,
			sending_apex TEXT NOT NULL,
			touch_number INT NOT NULL CHECK (touch_number >= 0),
			isp TEXT NOT NULL CONSTRAINT capdec_isp_vocab CHECK (isp IN
				('gmail','yahoo','aol','microsoft','apple','comcast','charter','att',
				 'sbcglobal','cox','verizon','protonmail','zoho','other')),
			base_cap INT NOT NULL CHECK (base_cap >= 0),
			preclaim_cap INT NOT NULL CHECK (preclaim_cap >= 0),      -- cap entering claimRecordsByISPCaps
			post_throughput_cap INT NOT NULL CHECK (post_throughput_cap >= 0),
			final_cap INT NOT NULL CHECK (final_cap >= 0),
			eligible_inventory INT CHECK (eligible_inventory IS NULL OR eligible_inventory >= 0),
			claimed INT NOT NULL DEFAULT 0 CHECK (claimed >= 0),
			deferred INT NOT NULL DEFAULT 0 CHECK (deferred >= 0),
			stage_vector JSONB NOT NULL,             -- full ordered vector [{"stage":"base","cap":100},...]; deleted key -> cap -1
			binding_stage TEXT NOT NULL CONSTRAINT capdec_stage_vocab CHECK (binding_stage IN
				('none','base','governed_floor','property_governor','warmup_keeponly','daily_budget',
				 'brand_routing','intro_budget','apple_ban','yahoo_newsletter','followup_daily_budget',
				 'claim','throughput_safety')),      -- DERIVED convenience; stage_vector is authoritative
			binding_value INT NOT NULL,
			campaign_id UUID,                        -- NULL legitimate on no-campaign exits (§10.1)
			organization_id UUID NOT NULL,           -- stamped at emission from po.cfg.OrganizationID
			PRIMARY KEY (id, decided_at),
			CONSTRAINT capdec_attempt_isp_uniq UNIQUE (wave_attempt_id, isp, decided_at)
				-- decided_at required by the PG partitioned-unique rule; dedup preserved because
				-- decided_at is frozen once per attempt and carried through every retry
		) PARTITION BY RANGE (decided_at)`},
		{"capdec_create_default_partition", `CREATE TABLE IF NOT EXISTS partner_drip_cap_decisions_default
			PARTITION OF partner_drip_cap_decisions DEFAULT`},
		{"capdec_idx_decided", `CREATE INDEX IF NOT EXISTS idx_drip_capdec_decided ON partner_drip_cap_decisions (decided_at DESC)`},
		{"capdec_idx_dataset", `CREATE INDEX IF NOT EXISTS idx_drip_capdec_dataset ON partner_drip_cap_decisions (dataset_id, decided_at DESC)`},
		// Daily cap X-ray aggregate (§5.8b): written by the 'xray_daily' pass
		// from partner_drip_cap_decisions; the portal reads THIS by default,
		// raw decisions only on drill-down.
		{"dob_create_cap_xray_daily", `CREATE TABLE IF NOT EXISTS partner_drip_cap_xray_daily (
			run_id UUID NOT NULL REFERENCES partner_drip_observatory_runs(run_id),
			organization_id UUID NOT NULL,
			day DATE NOT NULL,
			dataset_id UUID NOT NULL,
			pass TEXT NOT NULL CONSTRAINT xrayd_pass_vocab CHECK (pass IN ('welcome','followup')),
			brand_code TEXT NOT NULL,
			sending_apex TEXT NOT NULL,
			isp TEXT NOT NULL CONSTRAINT xrayd_isp_vocab CHECK (isp IN
				('gmail','yahoo','aol','microsoft','apple','comcast','charter','att',
				 'sbcglobal','cox','verizon','protonmail','zoho','other')),
			attempts INT NOT NULL CHECK (attempts >= 0),
			claimed INT NOT NULL CHECK (claimed >= 0),
			deferred INT NOT NULL CHECK (deferred >= 0),
			min_final_cap INT NOT NULL CHECK (min_final_cap >= 0),
			max_final_cap INT NOT NULL CHECK (max_final_cap >= 0),
			binding_stage_counts JSONB NOT NULL DEFAULT '{}'::jsonb,
			computed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
			PRIMARY KEY (organization_id, day, dataset_id, pass, brand_code, isp, run_id)
		)`},
		// Typed campaign-meta companion (§5.11): the TABLE ships now (schema-
		// before-binary); the stampPartnerAttributionOnCampaign WRITER is the
		// HOLD-CRITICAL D3b change (§7.0) and does NOT ship here.
		{"dob_create_campaign_meta", `CREATE TABLE IF NOT EXISTS partner_drip_campaign_meta (
			campaign_id UUID PRIMARY KEY,
			dataset_id UUID NOT NULL,
			brand_code TEXT NOT NULL,
			sending_apex TEXT NOT NULL,
		...
		// Contract COMMENTs for the increment's tables (§3.8 discipline).
		{"dob_comments_link_audit", `COMMENT ON TABLE partner_drip_link_audit IS 'Semantic link-attribution audit facts (§5.6/§7.2), one generation per link_audit run — swapped ONLY by the link_audit pass, never the rollup. row_kind=host rows audit money-host clicks per (campaign, day, host); the row_kind=coverage row accounts for every click row so audited+skipped+parse_error reconcile against total coverage. NO_COVERAGE and query-parse errors are RED, never silently green.';
			COMMENT ON COLUMN partner_drip_link_audit.event_day IS 'The clicks'' event_at Denver day.';
			COMMENT ON COLUMN partner_drip_link_audit.status IS 'GREEN | RED | NO_COVERAGE — no default: GREEN must be earned explicitly by the audit pass (rev-4.1 letter correction of the rev-3 GREEN-by-default).';
			COMMENT ON COLUMN partner_drip_link_audit.row_kind IS 'host = one audited money host; coverage = the campaign-day coverage-accounting row (url_host empty). No default (§5.0).';
			COMMENT ON COLUMN partner_drip_link_audit.brand_code IS 'Via brandident; empty string when identity is quarantined — the row is still audited (sub2 checks skip, §7.2).';
			COMMENT ON COLUMN partner_drip_link_audit.sub2_invalid IS 'sub2 present but not equal to the campaign''s sending_apex (§0 audit rule: sub2 absence never REDs; mismatch does). <= sub2_present (CHECK).';
			COMMENT ON COLUMN partner_drip_link_audit.sample_url_redacted IS 'Redacted sample form ONLY (§7.2) — never a raw subscriber-identifying URL.'`},
		{"dob_comments_hygiene", `COMMENT ON TABLE partner_drip_hygiene_daily IS 'Hygiene facts (§5.7/§6.7) at dataset x cohort_day x population grain — NO brand columns (rev-4.1 correction). Two labeled populations with their OWN denominators, never merged: first_touch_dispatch (validation coverage + subscriber verification over the pcq dispatched cohort) and all_touch_events (bounce anatomy + complaints across ALL touches; subs_*/pcq_* stay 0).';
			COMMENT ON COLUMN partner_drip_hygiene_daily.population IS 'first_touch_dispatch | all_touch_events — the §7.1 split-population key, part of the unique index. No default.';
			COMMENT ON COLUMN partner_drip_hygiene_daily.subs_matched IS 'LATERAL-join canonical subscriber match (validated-first, oldest-import, id — §6.7). Hard identity: matched + unmatched = dispatched_total on first_touch_dispatch rows (dob_hygiene_match_identity).';
			COMMENT ON COLUMN partner_drip_hygiene_daily.bounce_anatomy IS 'hard/soft/reputation/validation as SEPARATE keys — never summed (METRIC_CONTRACT).'`},
		{"dob_comments_cap_decisions", `COMMENT ON TABLE partner_drip_cap_decisions IS 'Per-(wave_attempt x ISP) cap-decision trace (§5.8/§10) — RANGE-partitioned on decided_at; the migration ships parent + DEFAULT only, month partitions are created by the xray_maintenance op relative to the deployment date (§10.6). INERT until the P6 emission (HOLD-CRITICAL D6): nothing writes here yet. Single writer = the orchestrator emission; no column renames without cross-vector sign-off (§0).';
			COMMENT ON COLUMN partner_drip_cap_decisions.decided_at IS 'Partition key; frozen ONCE at trace creation and shared by every row of the attempt, so capdec_attempt_isp_uniq still dedupes duplicate batches (§10.1).';
			COMMENT ON COLUMN partner_drip_cap_decisions.stage_vector IS 'Full ordered stage vector [{"stage":...,"cap":...}] — the authoritative record; a stage that deleted the ISP key records cap -1. JSON-serialized only in the writer goroutine (§10.3).';
			COMMENT ON COLUMN partner_drip_cap_decisions.binding_stage IS 'DERIVED convenience (first stage that set the final value); stage_vector is authoritative. Vocabulary = the §10.2 canonical stage list.';
			COMMENT ON COLUMN partner_drip_cap_decisions.campaign_id IS 'NULL is legitimate on no-campaign exits (intro-budget-exhausted, zero-claimed) — §10.1.';
...
BEGIN
    FOR rec IN
        SELECT * FROM (VALUES
            ...
            ('144.225.178.56', 'mta-trb-yh8.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.57', 'mta-trb-yh9.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.58', 'mta-trb-yh10.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.59', 'mta-trb-yh11.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.60', 'mta-trb-yh12.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.61', 'mta-trb-yh13.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.62', 'mta-trb-yh14.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ('144.225.178.63', 'mta-trb-yh15.mail.em.theretirementblog.com', 'trb-yahoo-pool', '15.204.101.125', '144.225.178.0/25'),
            ...
            ('15.204.38.163', 'mta-usf-yh11.mail.em.us-finance.com', 'usf-yahoo-pool', '15.204.107.107', '15.204.38.160/28'),
            ('15.204.38.164', 'mta-usf-yh12.mail.em.us-finance.com', 'usf-yahoo-pool', '15.204.107.107', '15.204.38.160/28'),
            ('15.204.38.165', 'mta-usf-yh13.mail.em.us-finance.com', 'usf-yahoo-pool', '15.204.107.107', '15.204.38.160/28'),
            ('15.204.38.166', 'mta-usf-yh14.mail.em.us-finance.com', 'usf-yahoo-pool', '15.204.107.107', '15.204.38.160/28'),
            ('15.204.38.167', 'mta-usf-yh15.mail.em.us-finance.com', 'usf-yahoo-pool', '15.204.107.107', '15.204.38.160/28'),
            ...
            ('144.225.178.141', 'mta-pmd-yh8.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.159', 'mta-pmd-yh9.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.160', 'mta-pmd-yh10.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.161', 'mta-pmd-yh11.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.162', 'mta-pmd-yh12.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.163', 'mta-pmd-yh13.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.164', 'mta-pmd-yh14.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25'),
            ('144.225.178.180', 'mta-pmd-yh15.mail.em.paymydebit.com', 'pmd-yahoo-pool', '15.204.107.107', '144.225.178.128/25')
...
END $$`},
	...
	migrations = append(migrations, []struct {
		...
		// LAST on purpose — see nxWaveIPRepointSQL.
		{"nx_wave_ip_repoint_20260918_final_pass", nxWaveIPRepointSQL},
	}...)
