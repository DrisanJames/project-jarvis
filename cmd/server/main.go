...
func runStartupMigrations(db *sql.DB) {
	migrations := []struct {
		...
		{"reset_failed_queue_items", `UPDATE mailing_campaign_queue SET status = 'queued', worker_id = NULL, locked_at = NULL, retry_count = COALESCE(retry_count, 0) WHERE status IN ('failed', 'sending') AND retry_count < 3`},
	}
