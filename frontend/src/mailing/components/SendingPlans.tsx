    ...
    >
      <div className="plan-header">
        ...
        <div className="confidence-badge">
          {formatPercent(plan.confidence_score * 100)} confidence
        </div>
      </div>
      ...
      <div className="plan-metrics">
        <div className="metric">
          ...
          <span className="metric-value">{plan.recommended_volume.toLocaleString()}</span>
        </div>
        <div className="metric">
          <span className="metric-label">Est. Opens</span>
          <span className="metric-value">{plan.predictions.estimated_opens.toLocaleString()}</span>
        </div>
        <div className="metric">
          <span className="metric-label">Est. Clicks</span>
          <span className="metric-value">{plan.predictions.estimated_clicks.toLocaleString()}</span>
        </div>
        <div className="metric revenue">
          <span className="metric-label">Est. Revenue</span>
          <span className="metric-value">{formatCurrency(plan.predictions.estimated_revenue)}</span>
          <span className="metric-range">
            ({formatCurrency(plan.predictions.revenue_range[0])} - {formatCurrency(plan.predictions.revenue_range[1])})
          </span>
        </div>
      </div>
          ...
          <div className="time-slots">
            {plan.time_slots.map((slot, i) => (
              <div key={i} className="time-slot">
                ...
                <span className="slot-volume">{slot.volume.toLocaleString()} emails</span>
                <span className={`slot-priority priority-${slot.priority}`}>{slot.priority}</span>
              </div>
            ))}
          </div>
            ...
            {plan.audience_breakdown.map((segment, i) => (
              <div key={i} className="audience-segment">
                ...
                <div className="segment-metrics">
                  <span>Open Rate: {formatPercent(segment.predicted_open_rate)}</span>
                  <span>Click Rate: {formatPercent(segment.predicted_click_rate)}</span>
                </div>
                <div className="segment-action">{segment.recommended_action}</div>
              </div>
            ))}
          </div>
        </div>
